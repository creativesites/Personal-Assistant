...
**Deferred to `docs/CAREER_GROWTH_ENGINE_PLAN.md`'s later phases**, per this codebase's established "ship a real slice, document the rest" discipline: AI Resume Studio (multi-document generation from the one master profile), Intelligent Job Discovery with Zambia-first localisation, the AI Application Engine and Interview Coach, match scoring (`career_opportunities.match_score`/`match_breakdown` exist as columns with no writer yet), the Relationship-to-Opportunity Bridge ("shortest path" introduction suggestions), Career Radar (the 0–100 opportunity-readiness composite score), and Career Coach/Motivation & Accountability — none built in Phases 1–2.
