...
See `docs/MEMBERSHIP_PLATFORM_PLAN.md` for the full design — a from-the-ground-up rebuild of billing as a Subscription & Membership Platform for Zambia's mobile-money-first market, replacing the earlier 6-tier single-cadence pricing (migration `0073`, `docs/PRICING_PAYMENTS_PLAN.md`) with 5 feature-based families (Free → Personal K149 → Professional K249 → Business K499 → Enterprise/custom), each sellable across up to 4 billing cadences (daily/weekly/monthly/yearly), a 30%-off BYOK (bring-your-own-API-key) discount, a guided 4-step manual mobile-money payment flow (Airtel `0979046745` / MTN `0762368105`, both registered to Winston Zulu), a 7-day full-access trial, grace-period/read-only lifecycle (never a hard lockout), a promotion/growth layer (referrals, gifts, promo codes, student verification), org seat limits, and an admin Revenue Intelligence dashboard. Shipped across 9 phases, each migration-tested + typechecked + committed independently, per this codebase's established engine-build discipline.

**Phase 1 — Membership Engine foundation** (migration `0083`): `subscription_plans` gains `plan_family` (`free|personal|professional|business|enterprise`), `billing_period` (`trial|daily|weekly|monthly|yearly`), `price_ngwee_byok`, `is_custom_pricing`, `trial_days`, `grace_period_days` (default 7), `included_seats`, `included_features` (`TEXT[]` of feature-area keys), and `documents_per_day` (a 4th daily counter alongside the existing three). The three still-relevant monthly rows from migration `0073` are renamed *in place* (`monthly_personal`→`personal_monthly`, etc. — same row id, so `subscriptions.plan_id` FKs keep resolving with no repoint needed) and re-priced to the agreed K149/K249/K499; `daily_pass`/`weekly_pass` are flipped `is_active=false` (retired from future purchase, current holders finish their period on old terms) rather than deleted. New sellable daily/weekly/yearly rows complete the multi-cadence catalog (yearly = 10× monthly, weekly ≈ monthly/4 × 1.15, daily ≈ weekly/7 × 1.3 — seed data, tunable later without a migration). `subscriptions` gains `billing_period`, `uses_own_api_key`, `grace_period_ends_at`, `read_only_at`, `documents_remaining_today`. New append-only `subscription_events` (one row per lifecycle transition — `trial_started`, `payment_approved`, `entered_grace_period`, `referral_redeemed`, `plan_changed`, etc.) is the single source for the Billing Timeline and Revenue Intelligence, same "one audit table per domain" convention as `business_events`/`goal_events`. Two lifecycle bugs fixed in the same pass: `auth.ts`'s signup now actually sets `current_period_end = NOW() + 7 days` at signup (previously only `trial_ends_at` was set, so the lifecycle worker's expiry check silently never fired), and `credits.py`'s `CreditType` widened to include `'document'`, wired into `documents.ts`'s AI-generate endpoint via the same `try_consume_credit` pattern `message_worker.py` already used for the other three counters.

**Phase 2 — Entitlement Engine** (`services/api/src/lib/entitlements.ts`): a fixed `FEATURE_AREAS` set (`career_os`, `cv_studio`, `job_search`, `interview_coach`, `advisor_companion`, `business_os`, `teams`, `analytics`, `automation`, `enterprise_api`) mapped per plan family in `PLAN_FEATURES` — Free unlocks none of these (Personal CRM/Advisor/Documents/basic Career+Business tools stay ungated app-wide regardless of plan, metered instead by the numeric daily caps). `getEffectivePlanFamily()` reads the joined plan row's `plan_family`, with a `trialing`-status override hard-coded to `'business'` (`TRIAL_GRANTS_FAMILY`) — "all Premium features for 7 days," short of Enterprise's custom framing. `requireFeature(area)` is a Fastify preHandler mirroring `requireMarketingAccess`'s exact shape, 402 (a billing gate, not a permissions one) with an `upgradeRequired: {feature, currentFamily, requiredFamily}` payload the frontend's contextual-upgrade UI (Phase 6) renders directly — wired onto every Career/CV Studio/Job Search/Interview Coach route, every Business OS route (`studio.ts`, `products.ts`, `services.ts`, `projects.ts`, `suppliers.ts`, etc.), and `team.ts`/`analytics.ts`/`agents.ts`/`proactive.ts` for `teams`/`analytics`/`automation`. A global `readOnlyModeGuard` preHandler (registered once in `app.ts`, not per-route) blocks any mutating request (`POST|PUT|PATCH|DELETE`) once a subscription is `read_only`, except an explicit allowlist (`/api/auth`, `/api/subscriptions`, `/api/admin`, `/api/webhooks`, `/api/health`) — it runs its own best-effort `jwtVerify()` since a global hook executes before a route's own `authenticate` preHandler, letting an unauthenticated request fall through to the route's normal auth rejection instead.

**Phase 3 — Guided multi-step payment flow**: `POST /api/subscriptions/checkout` gained `billingPeriod`, `useOwnApiKey` (resolves `price_ngwee_byok` when the caller has a `byok_keys` row), plus (once Phase 7 landed) `promoCode`/`referralCode`. `payment_requests` gained `payer_phone_number`/`payer_paid_at` (self-reported) and a screenshot-upload path; a new confirm step persists these via `@fastify/multipart`. The frontend replaced the old single-shot checkout button with `GuidedPaymentModal` (`billing/_components/guided-payment-modal.tsx`) — plan+period+BYOK toggle → network (Airtel/MTN) → pay-to screen with Copy Number/Copy Reference/"I Have Paid" → confirm (phone/time/optional screenshot) → an "estimated confirmation: 5–30 minutes" waiting state — reused identically by the Gift flow in Phase 7.

**Phase 4 — Grace period, read-only mode, notifications**: `subscription-lifecycle-worker.ts` now runs the full `active/trialing → grace_period → read_only` chain (writing a `subscription_events` row at each transition) instead of a single hard `expired` jump — full feature access continues through the grace period (`grace_period_days`, default 7); only `read_only` trips Phase 2's mutation guard. A new minimal `services/api/src/routes/notifications.ts` (`GET /api/notifications`, `PATCH /api/notifications/:id/read`) finally serves the `/notifications` page's pre-existing calls, which previously 404'd. The worker writes `notifications` rows at 7/3/1 days before expiry, on trial end, on entering grace period, and on the grace period's last day — deduped via a recency check on `subscription_events` so no threshold fires twice. A shared `useSubscriptionStatus()` hook and a persistent (non-dismissible) banner disable create/generate actions app-wide once `read_only`, while view/export/search stay fully available everywhere.

**Phase 5 — Premium billing dashboard**: `GET /api/billing/usage-summary` is a compute-on-read endpoint (documents generated, AI conversations, projects, customers, job searches, interviews, invoices — all scoped to the current billing period) feeding both the Usage cards and a small, explicitly-labeled "hours saved" estimate; `GET /api/billing/timeline` merges `subscription_events` + `payment_requests` into one chronological feed, same `UNION ALL` convention `reflection.ts`'s timeline already used. `/billing` was rebuilt in the Zuri gradient/card design system: `MembershipCard` (plan/status/valid-until/next renewal amount), `ProgressRing` (days remaining, SVG), `UsageCards`, `BillingTimeline`, and Upgrade/Renew/Manage buttons opening `GuidedPaymentModal`.

**Phase 6 — Contextual upgrade intelligence**: the session payload (`useZuriSession`/clerk-sync response) gained `planFamily` so the frontend can gate without a per-page round-trip. `FeatureGate` (previously dead code, never imported anywhere) was revived with a `requiredFamily` prop, wrapping every Phase-2-gated page's content in a locked `EmptyState` + exact upgrade-copy CTA. The "you've hit your daily AI limit" case — which `try_consume_credit` already enforced server-side but failed silently — now surfaces as an Inbox-style banner reusing the existing toast/notification conventions.

**Phase 7 — Promotion Engine** (migration `0086`): four features sharing the Promotion Engine name. **Referrals** (`referral_codes`/`referral_redemptions`) reward +14 days to both parties, but only once the *referred* user's first payment is admin-*approved* — never at signup or checkout — the plan's own explicit anti-abuse design; recording (at checkout, `ON CONFLICT (referred_user_id) DO NOTHING`) and rewarding (inside `admin-payments.ts`'s approve transaction) are deliberately two separate moments. **Gift memberships** (`gift_memberships`) reuse the exact `GuidedPaymentModal`/admin-approval pipeline with zero new payment infrastructure: a gift creates a normal `payment_requests` row under the *gifter's own* `subscription_id` (a schema constraint, not semantically "for" that subscription), and `admin-payments.ts`'s approve/reject handlers detect a linked `gift_memberships` row and branch — approval flips the gift to `status='ready'` (generating a `redemption_code`) instead of touching the gifter's own subscription; a new `/redeem/:code` page applies it to the recipient's account. **Promo codes** (`promo_codes`/`promo_code_redemptions`) validate at checkout (active window, plan-family restriction, redemption cap) and stack *on top of* whichever base price (standard or BYOK) was already resolved, never on the undiscounted price; `admin-promotions.ts` gives full CRUD. **Student verification** (`student_verifications`) unlocks a hidden 50%-off `personal_monthly_student` catalog row (`is_active=false`, so it never appears in the public catalog) — enforced server-side at checkout by checking `users.is_verified_student` directly, not relying on catalog obscurity alone, matching this codebase's "don't rely on hiding it" discipline elsewhere.

**Phase 8 — Org seats + Admin Revenue Intelligence**: `team.ts`'s invite endpoint now checks the inviting team's `team_members` count against the owner's active plan's `included_seats` before adding a member, 402'ing with an upgrade payload if exceeded (dynamic seat-purchase-at-checkout math is deferred, documented scope). `GET /api/admin/revenue` (`services/api/src/routes/admin-revenue.ts`) computes MRR/DRR/WRR by normalizing every `active`/`grace_period` subscription's plan price (BYOK-aware) to a daily rate via `duration_days` then scaling — a subscriber on a weekly plan contributes correctly to MRR and vice versa — plus trial conversion rate (from `subscription_events`' `trial_started`/`payment_approved` rows), a 30-day churn rate (`entered_grace_period` events over the at-risk denominator), renewals due today, and failed renewals in the last 7 days; all deterministic SQL, no LLM call. `POST /api/admin/payments/match` is "Intelligent Payment Detection," honestly scoped as a text-paste parser rather than a live SMS integration (no such infrastructure exists — the Kotlin companion app's `NotificationListenerService` is the natural future extension point): an admin pastes the raw mobile-money confirmation text, a plain-code parser extracts amount/phone-fragment/reference-code and fuzzy-matches against pending `payment_requests`, returning a ranked confidence list for one-click approve via the existing approve endpoint. `POST /api/admin/users/:userId/adjust-subscription` (extend days and/or change plan) reuses `admin_audit_log` like every other admin mutation. The new `/admin/revenue` page renders the stat tiles, the paste-and-match tool, and a "subscribers by plan family" breakdown; the admin user detail page gained a "Membership adjustment" card for the extend/change-plan actions.

**Deviations from the original spec** (all deliberate): the manual-adjustment endpoint is keyed by `userId` rather than `subscriptionId` in the URL, since the admin user-detail page (the only frontend caller) has a user id, not a subscription id, on hand — the route resolves the user's current subscription server-side. Dynamic seat purchase at checkout, real SMS-reading payment detection, and a full admin plan-catalog editor are all explicitly out of scope for this build — documented as follow-on work in `docs/MEMBERSHIP_PLATFORM_PLAN.md`, not built here.
...
See `docs/BUSINESS_EVENTS_PLAN.md` for the full design. The unifying idea: `WhatsApp Message → Message Analysis → Business Intelligence Events → Action Bundle → User Approval → ERP Updated` — Zuri continuously extracts business structure from ordinary conversations rather than waiting to be told, bundles related detections into one approval instead of asking one question at a time, and proactively nudges a user toward formal business hygiene. Everything below is additive to the existing pipeline (`MessageAnalysis`, `action_bundles`, `contacts.customer_status`) — no second LLM pass, no new commercial entity, no new UI mechanism where an existing one already fit.

**Migration `0076`** adds `business_events` — a generic, durable log of every detected business signal (`event_type`, `contact_id`/`conversation_id`/`message_id`, `confidence`, `evidence` JSONB string array, `payload` JSONB, `bundle_id`, `status` pending/bundled/dismissed/expired) independent of whether it produced a user-facing action. Named `business_events` rather than "business intelligence events" specifically to avoid colliding with the pre-existing Phase 9 "Business Intelligence & Executive Intelligence Platform" analytics dashboards — the conceptual name in prose stays "Business Intelligence Events," the table/service name is shorter. `services/intelligence/app/services/business_events.py`'s `BusinessEventService` is a pure `record()`/`mark_bundled()` insert with no side effects — callers decide what to do with the returned id. The same migration adds `action_bundles.confidence`/`evidence` columns (populated as a bundle is assembled, so `ActionBundleCard` can show a "78% confident — because: ..." expandable line instead of a bare summary), widens `products.status`'s CHECK from `active|sold|archived` to `active|secondary|archived|discontinued` (`'sold'` backfilled to `'archived'` — confirmed dead via grep before dropping), and adds `advisor_user_profiles.business_manager_paused BOOLEAN NOT NULL DEFAULT FALSE`.

**New detectors, added to the existing per-message `MessageAnalysis` call** (`services/intelligence/app/models.py`/`ai/prompts.py`'s `ANALYSE_MESSAGE`) rather than a second LLM pass: `new_products_mentioned` (a product/service mentioned that doesn't match anything in the catalog — the "create a product from chat" signal, with an `is_one_off` flag for the mechanic's-spare-part case) and `suppliers_mentioned` (a supplier not already in `suppliers`). Unlike `order_intent_mentioned`, neither is gated on `sender_type == 'contact'` — a business owner saying "I sourced a part for this job" or "I bought this from a new supplier" is itself a valid, expected signal source, not just something a customer says.

**Generalized `action_bundles`** (`services/intelligence/app/services/action_bundles.py`'s `ActionBundleService.detect_and_create`, called from `message_worker.py`): previously only consumed `order_intent_mentioned`; now also consumes `new_products_mentioned`/`suppliers_mentioned`, folding all three into **one** bundle per message-analysis pass rather than three separate approval cards — one message mentioning a new product, a new supplier, and an order produces one `action_bundles` row with `create_product`/`create_supplier`/`create_deal`/`reserve_stock`/`generate_document` actions, dependency-ordered via the existing `dependsOn` mechanism. Every detection above the confidence floor writes a `business_events` row unconditionally (the audit trail), even when a pending bundle for that contact already exists in the last 60 minutes and the detection doesn't get its own card — `business_events.bundle_id`/`status` are set once a detection is folded into a bundle. Two new action types, `create_product` (`POST /api/products`, always proposing `status: 'secondary', trackInventory: false` — the AI's own proposal starts conservative) and `create_supplier` (`POST /api/suppliers`), were added to `apps/web/src/lib/action-executor.ts`'s `BundleAction['type']` union and `ActionBundleCard`'s executor; the `{type, params, dependsOn?}` JSONB shape itself needed no schema change.

**Product lifecycle** — `products.status`'s widened vocabulary makes `secondary` the mechanic's-spare-part state: fully recorded (supplier, cost, price, and via `contact_products`, who it was sold to) but excluded from the Catalog tab's default grid. `GET /api/products` accepts `?includeSecondary=true` (`services/api/src/routes/products.ts`) to include it; the Catalog tab (`studio/_components/catalog-module.tsx`) fetches with the flag unconditionally and does the show/hide filtering client-side via a `visibleProducts` computed value plus a "Show/Hide secondary (N)" toggle, so the toggle is instant and the count is always accurate — a violet "secondary" badge marks these cards, with a one-click "Promote to active" `PATCH {status: 'active'}` in the expanded detail. `discontinued` is new and distinct from `archived` — "no longer available but keep the history for reporting" vs. hidden/old data.

**Customer Management** — deliberately **not** a new table. `contacts.customer_status = 'customer'` (migration `0021`) is already the entity; this is a Studio-side commercial lens over the same row, the same "siblings, not parallel schemas" reuse discipline already applied to Services vs. Products. `GET /api/studio/customers` (`services/api/src/routes/studio.ts`) joins the proven `revenue_events` per-contact `LATERAL` pattern (from `relationships.ts`) with a new per-contact paid/outstanding-invoice `LATERAL` join on `documents` and the existing `contact_products` ⋈ `products` purchase-history join, computing lifetime value (`revenue_events` + paid invoices), outstanding balance (sent/viewed/downloaded invoices), purchase count/history, last purchase date, an on-read `gold`/`silver`/`bronze` tier (a handful of rows per page load, not a hot path — no denormalized column needed), and an `atRisk` flag reusing `relationships.health_trend = 'declining'`. `apps/web/.../studio/_components/customers-module.tsx` is the new `CustomersModule`, wired into `studio/page.tsx`'s `Module` union/`MODULES` array/`renderModule` switch the same 3-line way the Services tab was added.

**Studio module extraction** — as part of this effort, `CatalogModule` and `ServicesModule` (previously both inline in `studio/page.tsx`, which had grown to ~5,000 lines) were extracted into their own files under `studio/_components/` per the new "File Architecture" convention below, alongside `CustomersModule` being born there from the start; shared types/helpers (`Product`, `Supplier`, `formatCurrency`, `PRICING_MODEL_LABELS`, etc.) moved into `studio/_components/shared.ts` so all three module files and `page.tsx` itself import from one source of truth instead of duplicating them.

**Business Manager Assistant** (`services/intelligence/app/services/business_manager.py`) — a proactive, toggleable nudge layer living throughout the system, not a new detection or delivery mechanism. Gated on `advisor_user_profiles.business_manager_paused = false`, exposed via the existing `GET/PATCH /api/advisor/profile` (`services/api/src/routes/advisor.ts`) — same "paused=false means on by default, honest kill switch" precedent as `companion_features_paused`, with a real toggle switch on Settings' AI Engines tab (previously that tab's engine list was purely decorative/subscription-gated; this is the first genuinely wired toggle there). Fixed alongside: `companion_features_paused` itself had a latent bug where the PATCH handler cast it (and now would have cast `business_manager_paused`) with `::jsonb` inside the same loop as the actual JSONB profile fields, despite both being plain `BOOLEAN` columns — split into a separate `booleanColumns` loop with a correct `::boolean`-compatible assignment. Phase 1 ships one concrete behavior, deliberately narrow — "completed work, no invoice": a project crossing the same ≥75% task-completion threshold `project_progress.py` already tracks, OR a `deals.stage = 'closed_won'`, with no `invoice`/`quotation` document linked to that contact in the last 30 days, surfaces as an approvable "want Zuri to draft one for your records?" suggestion in the existing `proactive_queue` dock (reusing the `follow_up` suggestion type rather than adding a new enum value) — deduped via a `business_events` row (`event_type = 'invoice_gap'`, keyed on `projectId`/`dealId` in `payload`) doubling as both the dedup marker and the audit-trail entry. Deliberately not an AI call, same reasoning as `project_progress.py`/`document_followups.py`. Wired into `daily_worker.py`/`main.py` as a seventh daily scheduler at 18:00 UTC, the next free slot after Project Management's 17:00 check. Every other Business Manager behavior from the wider brainstorm (stale supplier comparisons, "customer became a top client," overdue follow-ups) is documented as a same-shaped follow-on cron in `docs/BUSINESS_EVENTS_PLAN.md` §9 Part G, not built in this pass.

**"Zuri Noticed" activity feed** — `GET /api/studio/insights` gained a `recentEvents` array (the last 10 `business_events` rows for the user, joined to the contact's name) rather than a new endpoint, same "extend the existing insights payload" convention `stockoutForecasts`/`topProfitable` already followed. Rendered as a new card on Studio's `OverviewModule`, alongside the existing Zuri Insights cards and the Financial Overview — each row shows the event type, the detected name/company, the first evidence bullet, a confidence percentage, and a "In pending bundle" badge when `status = 'bundled'`.

**Deferred to `docs/BUSINESS_EVENTS_PLAN.md` §9 Part G**, per this codebase's established "ship a real slice, document the rest" discipline: expense/payment detection (`ExpenseMention`/`PaymentMention`), a company assets register, a full staged progressive-formalization UI (the "Zuri Noticed" feed is the honest, lightweight version), deeper product/service intelligence beyond what already exists (co-purchases, replacement-date prediction), a generic condition→action Automation Engine, and the broader Business Manager behavior library beyond the one shipped behavior.

---

## Zuri Reality Engine

See `docs/REALITY_ENGINE_PLAN.md` for the full design — a new core Neural Layer engine (§4.11 of `docs/NEURAL_LAYER_PLAN.md`) that answers the question none of the other engines ever asked: *is what Zuri already concluded still true?* If the Memory Engine answers "what do I know?" and the Prediction Engine answers "what's likely to happen?", the Reality Engine answers "is what I know still correct?" — closing the gap between a system that accumulates intelligence and one that maintains it. Named Reality Engine rather than "Reconciliation Agent" (the working title) since its job is broader than accounting-style reconciliation.

**No new generic table** — Phase 1 reuses `business_events` (migration `0076`) as the Reality Engine's own log, with new `event_type` values (`nudge_auto_resolved`, `contradiction_invoice_paid_deal_open`, `contradiction_negative_inventory`, `contradiction_project_complete_tasks_incomplete`) rather than inventing a parallel "claims ledger" — Studio's existing "Zuri Noticed" feed surfaces Reality Engine activity for free, and `business_events.confidence`/`evidence` carry the "Truth Sources" framing. **Migration `0077`** adds only what was genuinely missing: `proactive_status` (a real Postgres ENUM) gains `'auto_resolved'` — the one lifecycle terminal state meaning "Zuri determined this nudge is no longer relevant," distinct from `dismissed` (a human said no) and `sent` (a human acted on it); `proactive_queue` gains `resolved_reason` (the human-readable "why," the raw material for "learn from corrections") and `business_event_id` (an FK linking a nudge to the `business_events` row that spawned it, so `business_manager.py`'s invoice-gap nudges can be resolved by exact match rather than fuzzy title-text matching — set going forward only, existing pending rows are gracefully skipped); and `advisor_user_profiles.reality_engine_paused` (same on-by-default kill-switch precedent as `companion_features_paused`/`business_manager_paused`). `action_bundles.status`/`business_events.status`/`gossip_worthy_events.status` needed **no migration** — all three already declared `'expired'`/`'dismissed'` CHECK values that no code had ever written; Phase 1 finally writes to them.

**Three cadences, not one daily cron.** *Layer 1 (event-driven)*: `services/intelligence/app/services/reality_engine.py`'s `RealityEngineService.resolve_relationship_nudges()` is called from `message_worker.py`'s existing live-message branch (`sender_type == 'user'`, inside the same `not is_historical` gate the watch-narration check already uses) — a live outbound reply immediately auto-resolves any pending `check_in`/`follow_up`/`reconnect` nudge for that contact, records a `business_events(event_type='nudge_auto_resolved')` audit row, and fires `publish_event('reality.resolved:{userId}', ...)`. A parallel Node-side hook, `services/api/src/lib/reality-engine.ts`'s `resolveInvoiceGapNudges()`, is called directly from `documents.ts`'s `POST /:id/status` (`status === 'paid'` branch) and `deals.ts`'s `PATCH /:id` (`stage === 'closed_won'` branch) — both already own the write synchronously, so this is a direct Postgres UPDATE from the route itself rather than a new Node→Python call, matching this codebase's established `credits.py` precedent of direct DB access over a new HTTP round-trip; it publishes via the already-exported `publishInboxEvent()` (`inbox-events.ts`), reusing the same Redis instance/channel convention Python's `publish_event` already uses — no new plumbing, just one line added to `redis-subscriber.ts`'s psubscribe list. *Layer 2 (hourly)*: `run_hourly_sweep()` runs three deterministic, no-LLM-call SQL contradiction checks (invoice paid + deal still open, negative inventory with nothing incoming, a project marked `completed` with incomplete tasks) — detected and logged to `business_events` only, **never auto-mutated**, since Reality Engine only ever resolves Zuri's own artifacts, never a user's own business record. Wired via `daily_worker.py`'s existing fixed-interval scheduler shape (`asyncio.sleep(3600)`, the same template `run_interest_cron_scheduler`/`run_world_knowledge_scheduler` already use for their own sub-daily cadences). *Layer 3 (daily)*: `run_daily_sweep()` is the "cognitive garbage collection" pass — sweeps genuinely abandoned `pending` rows across `proactive_queue`/`action_bundles`/`business_events`/`gossip_worthy_events` into their `expired`/`auto_resolved` terminal states, wired as an eighth daily scheduler at 19:00 UTC, the next free slot after Business Manager's 18:00.

**Intelligence Health Score** — `GET /api/diagnostics/intelligence-health` (`services/api/src/routes/diagnostics.ts`), computed live per page load (a handful of aggregate queries, same "not a hot path" judgment already made for Studio's Customer tiers/Financial Overview): prediction freshness (% of `inventory_forecasts` rows with `computed_at` within 2 days — the one table in the schema with a ready-made freshness timestamp), relationship freshness (% of `relationships` rows updated within 7 days), nudge accuracy — the "learn from corrections" metric — (the fraction of `proactive_queue` rows over the last 30 days a user actually acted on vs. ones Reality Engine had to auto-resolve because reality moved past them), open contradictions, and one documented weighted overall score. Rendered as a new mobile-first stat-tile section on `/diagnostics`, matching the Token Usage Tracking section's exact established visual convention. The Settings toggle (`reality_engine_paused`) sits directly below the Business Manager Assistant toggle, same visual pattern; `/proactive`'s frontend listens for the new `reality.resolved` socket event (mirroring the Inbox's existing `bundle:ready`/`suggestion:ready` listener pattern) and refetches rather than guessing which specific row to remove client-side, since the event payload only carries a contact + count, not which suggestion types were touched.

**Deferred to `docs/REALITY_ENGINE_PLAN.md` §11**, per this codebase's established "ship a real slice, document the rest" discipline: per-field freshness columns beyond nudges/invoice-gap (health scores, lead scores, replacement-date predictions each already have *some* timestamp, but a real staleness-detection pass per field is its own design exercise); confidence auto-tuning from correction history (Phase 1 only *surfaces* the nudge-accuracy ratio, doesn't yet feed it back into detector confidence); contradiction auto-*fix* (today strictly detect-and-surface, per the safety boundary above); extending checks beyond Studio/business modules to relationship/Advisor-side contradictions; and a full Truth Sources UI on individual records (today "truth sources" live in `business_events.evidence` strings, not a dedicated per-record panel).

---

## Zuri Career & Growth Engine

See `docs/CAREER_GROWTH_ENGINE_PLAN.md` for the full design — Career OS: a parallel Career Graph and unified Opportunity Engine (jobs/contracts/consulting/grants/speaking/tenders/etc. as one object, only the workflow differs) built on the same Neural Layer substrate as everything else, aimed first at Zambian job seekers (remote and local) but architected to scale globally. Positioned as a growth/subscription driver, not a bolt-on "job search" feature — it reuses `relationship_connections`, `knowledge_graph_edges`, and `goal_profiles` as-is rather than inventing parallel graph/goal machinery, per the plan's own research into which existing tables are already unconstrained enough to carry professional-relationship semantics for free.

**Phase 1 — Foundation** (migration `0078`, see plan §3/§5): `career_profiles` (one row per user — headline, summary, skills/certifications/education/languages as JSONB, target roles/industries, salary expectation, remote/relocation preference, portfolio/GitHub/LinkedIn URLs) is the single professional-identity source every downstream feature (resume generation, match scoring, Career Radar) reads from. `career_opportunities` is the unified Opportunity Engine object — one `category` enum (`job|contract|consulting|investment|speaking|partnership|collaboration|freelance|board_position|research|mentorship|grant|scholarship|tender|supplier_opportunity|acquisition`) with a real status lifecycle (`detected→shortlisted→applied→interviewing→offered→accepted/rejected/withdrawn→archived`), deliberately a new table rather than reusing `opportunities` (whose `contact_id NOT NULL` and relationship-nudge-flavored `opportunity_type` vocabulary don't fit a role with no known contact). `career_interviews` tracks per-opportunity interview rounds (type, scheduled time linked to `calendar_events`, questions asked, AI feedback, outcome) — built now since Phase 1 already ships the opportunity lifecycle those rows hang off of, even though the AI Interview Coach itself is later-phase scope. Professional relationship types (`recruiter_for`, `mentor_of`, `colleague_at`, `referred_by`, `hiring_manager_for`) need no schema change — `relationship_connections.connection_type` and `knowledge_graph_edges`'s entity-type columns are unconstrained `VARCHAR`s (comment-documented conventions only), so the Career Graph is a lens over the existing graph tables, not a parallel structure. Career goals reuse `goal_profiles`/`goal_linked_entities` as-is — only the linked-entity CHECK widened to include `'career_opportunity'`. `relationships.career_signals` is a denormalized JSONB summary column, computed by `services/intelligence/app/services/career_signals.py`'s `CareerSignalsService.recompute()` on the exact same every-5th-message cadence `network_value.py`/`lead_score.py` already run on inside `message_worker.py` (not inside `health.py` itself — that's not where those two siblings live either). `services/api/src/routes/career-profile.ts` (`GET/PATCH /api/career/profile`) and `career-opportunities.ts` (full CRUD + status transitions, `SELECT_OPPORTUNITY` joining the contact name the same way every other contact-linked table does) are the API surface; the new `/career` page (added to the ungated nav group alongside Goals/Timeline, since career growth spans both business and personal modes) ships a profile hero card plus a status-filtered opportunity list with inline status transitions.

**Phase 2 — Passive Detection & Bundling** (no new migration, see plan §6): a friend saying "we're hiring React devs" or the user mentioning their own job search now surfaces the same way a new product or supplier mention does — folded into the existing per-message `MessageAnalysis` pass rather than a second LLM call. `CareerOpportunityMention` (`models.py`) and its `career_opportunities_mentioned` field on `MessageAnalysis` follow the exact `new_products_mentioned`/`suppliers_mentioned` template: not gated on `sender_type` (a friend's tip and the user's own mention are equally valid signals), narrow/high-confidence extraction only (a specific, named opportunity — a role, a contract, a grant — not vague "things are busy at work" chat), with `category` inferred from context and `is_remote` only set when explicit. `services/intelligence/app/services/action_bundles.py`'s `ActionBundleService.detect_and_create` gained a third mention type alongside orders/new-products/suppliers: a resolved career opportunity mention writes a `business_events(event_type='career_opportunity_detected')` audit row (deduped against the user's own non-terminal `career_opportunities` by title match, same discipline as the new-product/supplier dup checks) and folds a `create_career_opportunity` action into the same one-bundle-per-message-pass Inbox card — `category` values outside the DB's CHECK vocabulary (the LLM's own `'other'` escape hatch) fall back to `'job'` rather than failing the insert. `apps/web/src/lib/action-executor.ts` and `action-bundle-card.tsx` gained the matching `create_career_opportunity` type/label/executor (`POST /api/career/opportunities` with `source: 'whatsapp_detected'`) and bundle-title branch, reusing the exact `{type, params}` dispatch every other bundle action type already uses — no new execution mechanism. `GET /api/career/activity` (`career-opportunities.ts`) is a career-scoped slice of the same `business_events` read Studio's "Zuri Noticed" feed already does (filtered to `event_type = 'career_opportunity_detected'`, no new audit table), rendered as a "Zuri Noticed" card on `/career` showing the detected title/company, first evidence quote, confidence %, and an "in pending bundle" badge — the same feed pattern this codebase now uses in three places (Studio, Advisor, Career) over one shared `business_events` log.

**Phase 3 — AI Resume Studio** (migration `0079`, see plan §8): `documents.document_type`'s CHECK widens to add `resume`/`cover_letter`/`portfolio_page` — the same widen precedent `products.status` used this session — with the pre-existing `hr` `document_category` bucket (migration 0043) already fitting all three, no category widen needed. Generation is a sibling to `document_generator.py` rather than a reuse of it: `services/intelligence/app/services/resume_studio.py`'s `generate_resume_data()`/`generate_cover_letter_data()` are fed from `career_profiles` (not `business_profiles`) and produce a genuinely different structured_data shape (experience/education/skills, no items/pricing) that `generate_document_data()`'s items/sections/totals pipeline was never built to hold. `create_resume_document()` inserts the `documents` row directly (`contact_id NULL`, `document_category='hr'`) rather than routing through `create_document_row()`, which hard-codes the line-item insert. Rendering is likewise a new, dedicated path: `services/api/src/lib/pdf/templates/Resume.tsx` (a deliberately ATS-plain single-column layout — the layout choice that most affects whether an applicant-tracking system can parse it; Modern/Executive variants are deferred) and `CoverLetter.tsx` (a plain business-letter layout), with `document-render.ts`'s `renderAndSaveDocument()` branching to a separate `renderAndSaveResumeOrCoverLetter()` helper for these two document types rather than threading nulls through the business/contact-shaped `buildBusinessContext()`/`buildDocumentContext()`. **Resume analysis** (upload an existing CV, get scored) is `score_resume_text()` — a structured `SCORE_RESUME` prompt returning `atsCompatibility`/`recruiterAppeal`/`technicalStrength`/`achievementFraming`/`formatting`/`overallScore` plus concrete rewrite suggestions (the "responsibilities → achievements" pattern the plan calls out by name) — fed by `extract_resume_text()` (`pdfplumber` for PDF, direct decode for plain text, the same library `knowledge_retriever.py` already uses for KB documents). An uploaded resume's own PDF bytes become the storage artifact directly (`resume_studio.py`'s `_storage_path_for()`, writing to the same shared `doc_storage` Docker volume both `api` and `intelligence` containers mount) — no react-pdf re-render, which would discard formatting the person already chose. **CV-to-opportunity matching** (`match_resume_to_opportunities()`) reuses `documents.embedding` exactly as Business Workspace Phase 4 designed it, embedding each of the user's non-terminal `career_opportunities` (capped at 20, an honest scope reduction pending a dedicated `career_opportunities.embedding` column if this ever becomes a hot path) and ranking by cosine similarity. `services/intelligence/app/routes/career.py` (`/internal/career/resume/generate`, `/cover-letter/generate`, `/resume/score`, `/resume/upload`, `/resume/{id}/match`) mirrors `routes/documents.py`'s internal-route shape; `services/api/src/routes/career-documents.ts` is the Node-side surface (`POST /api/career/resume`, `/cover-letter`, `/resume/upload` via `@fastify/multipart`, `/resume/:id/match`, `GET /api/career/documents`), reusing `documents.ts`'s exported `formatDocument()`/`assignDocumentNumber()` rather than duplicating them. The new `ResumeStudio` component (`career/_components/resume-studio.tsx`, extracted per the File Architecture convention once `/career` started approaching 500 lines) renders the generate/upload/match/score UI as its own card on `/career`.

**Phase 4 — Applications as Projects + Interview Memory** (migration `0079`, see plan §9/§10): `projects.career_opportunity_id` is a nullable FK back to `career_opportunities` — `career_opportunities.project_id` already pointed the other way since migration 0078, the same both-directions-FK convention `documents.project_id`/`projects.deal_id` established for Business OS Phase B. `POST /api/career/opportunities/:id/apply` (`career-opportunities.ts`) is the identical "copy a workflow template into `projects`/`project_tasks`" convenience `POST /api/products/:id/start-project` already shipped for Services Management, reapplied here with the plan's own default task template (Tailor resume, Write cover letter, Submit application, Follow up in 7 days) — it moves the opportunity to `applied` and sets `project_id`; calling it again on an already-applied opportunity returns the existing project instead of duplicating one. `career_interviews` (already created in migration 0078) gets its full CRUD surface in the new `services/api/src/routes/career-interviews.ts`: round numbers auto-increment per opportunity, and a `scheduledAt` write also inserts a `calendar_events` row — a real schema detail the plan's own prose glossed over (every user gets a default "My Calendar" row at signup, `auth.ts`; `calendar_events` requires `calendar_id`, `start_time`, no `contact_id` column, distinct from the `events`/`event_type` table the main `/calendar` page and chat-tag reminders actually use — the two are separate, both-real tables, not one). "Company X tends to ask system design first" (`GET /api/career/interview-patterns?company=`) is a plain SQL aggregation over the user's own past `career_interviews` for that company (`interview_type` frequency, average `difficulty_rating`, past questions) — a plain read, not a new prediction model, matching the plan's own instruction. The new `OpportunityCard` component (`career/_components/opportunity-card.tsx`, extracted alongside Resume Studio) renders an "Apply — create project" action (swapping to a "View Project" link once `projectId` is set) and an expandable interview-round tracker with inline outcome updates, per opportunity card on `/career`.

**Phase 5 — Career Coach & Motivation** (no new migration, see plan §11): `companion_mode` gains `'career_coach'`, reachable the same way `gossip`/`spiritual_companion` already auto-switch on classified intent — `CLASSIFY_ADVISOR_TURN` gains a `career_advice` intent value, and both `handle_turn`/`handle_conversation_turn` in `advisor_companion.py` switch `companion_mode` to `career_coach` when it fires. Once active, the system prompt folds in a new `_career_context_line()` block — stated career goals, active (non-terminal) `career_opportunities`, and recent `career_interviews` rounds — read only when the mode is actually `career_coach`, matching this file's existing "don't pay for context the turn doesn't need" discipline (the same reasoning `_curiosity_context_line`/`_spiritual_verse_line` already follow). `career_coach.py` is not a new service — a `companion_delivery.py` consumer mirroring `motivational_detector.py`'s exact shape: plain SQL detection of three real signals (an opportunity stalled with no application movement in 14+ days, a rejection just logged, a move into `interviewing`/`offered`), one small LLM call only for the encouraging phrasing itself, enforced by a named `CAREER_COACH_TONE_POLICY` block (never sentimental or manipulative, always grounded in real counts) — the same "policy block over ad-hoc instruction" pattern `RELATIONSHIP_ADVICE_POLICY` already established. Wired as a ninth daily scheduler at 20:00 UTC, the next free slot after Reality Engine's 19:00 sweep. Deliberately **not** gated on `personal_mode_enabled` the way the other Phase 4.5 companion crons are — career growth spans business and personal modes alike, the same reasoning that keeps `/career` itself out of the mode-gated nav group — but it still respects the universal `companion_features_paused` kill switch and only fires for users who've actually engaged with Career OS (a `career_profiles` row exists).

**Phase 6 — Relationship-to-Opportunity Bridge** (no new migration, see plan §7): `shortestIntroductionPath()` (`services/api/src/lib/knowledge-graph.ts`) answers "who do I actually know who's closest to this opportunity?" — resolves the opportunity's hiring contact from a `knowledge_graph_edges` entry (`relation_type IN ('hiring_manager_for', 'recruiter_for')`, falling back to `career_opportunities.contact_id` directly when set), then runs a bounded (max depth 3) BFS over `relationship_connections` — bidirectional, since `contact_a_id`/`contact_b_id` carry no inherent direction — to the nearest contact the user actually has message history with (`relationships.last_interaction_at IS NOT NULL`, a more meaningful qualifying signal than a raw `health_score` threshold given that column's high default). No new schema: both graph tables already carried everything this needs. `GET /api/career/opportunities/:id/introduction-path` (`career-opportunities.ts`) exposes it and, once a path is found, calls the one genuinely new AI call the bridge needs — `career_networking.py`'s `generate_introduction_draft()`, reached via a new `/internal/career/introduction-draft` route — to draft the introduction-ask message itself. Deliberately **read/suggest-only**, per the plan's own instruction: Zuri never messages anyone on the user's behalf here, only drafts for the user to review and send. `OpportunityCard` renders the path and draft inline behind a "Who can introduce me?" button.

**Phase 7 — Career Radar + Reflection/Prediction/Reality integration** (no new migration, see plan §12/§14): `GET /api/career/radar` (`career-radar.ts`) computes a 0–100 composite score live per page load — the same "compute-on-read, not a hot path" judgment already made for the Intelligence Health Score and Studio's Customer tiers — from six sub-scores, each carrying its own one-line "why" and a concrete next action rather than a bare number: **Network** (average `health_score` of contacts with a `job_title`/`company` set, boosted by how many are flagged recruiter/hiring-manager/likely-referrer via `career_signals`), **Skills** (list completeness — see below), **Portfolio** (resume/portfolio-page document count + recency + external URLs on `career_profiles`), **Interview Readiness** (`career_interviews` outcome history with a recency penalty), **Market Demand** (matching `career_opportunities` detected in the trailing 90 days against target roles/industries — the plan's own named Phase 1 proxy), and **Visibility** (days since the most recent resume/portfolio-page update — also a named Phase 1 proxy). Skills is a deliberate simplification of the plan's own fuller vision: it measures list completeness (`skills.length`), not true relevance-to-target-role matching, which would need a cached AI skill-extraction pass per role title — real scope, not built here, and honestly labeled as such in the route's own file header rather than silently claiming a relevance read it doesn't compute. Rendered as a new `CareerRadar` card on `/career`, each sub-score tile expandable to show its why/next-action. Separately, three existing Neural Layer engines gained a Career adapter each, all pure reads of signals Phases 1/2/4/5 already produce: **Reflection Engine** (`neural/reflection.py`) gains a `career` highlight category (opportunities detected/applied, interview rounds logged this period); **Prediction Engine** (`neural/prediction.py`) gains `interview_success_likelihood`, a deterministic heuristic scoped first to the same company's past interview outcomes, falling back to the user's overall track record when there's no company-specific history yet; **Reality Engine**'s hourly sweep (`reality_engine.py`) gains two more contradiction checks in its existing detect-and-surface-only shape — `contradiction_stalled_application` (an opportunity stuck in `applied` for 21+ days) and `contradiction_interview_overdue_outcome` (an interview past its scheduled date with `outcome` still `pending`).

**Phase 8 — Localisation Depth** (migration `0080`, see plan §13/§16): the schema-level groundwork (`career_profiles.country`, `career_opportunities.country`) already shipped in Phase 1 — this phase is purely about seeding and wiring Zambia-first reference data, not new architecture. `career_employer_categories` is a small static reference table (employer name, aliases, category — `bank|telecom|mining|ngo|government|university|startup`, country), seeded with the plan's own worked examples (Zanaco, MTN Zambia, Barrick, ZRA) plus enough peers per category to be genuinely useful on day one — explicitly not a scraped directory, per the plan's own §17 deferral of multi-source job discovery. Wired into two concrete inference points, both reads only: `career_signals.py`'s `_lookup_employer_category()` enriches a contact's `career_signals.employerCategory` (an ILIKE substring match against `employer_name`/`aliases`, tolerant of "MTN Zambia Ltd"-style suffixed company names) right alongside the existing `currentCompany` computation; and Career Radar's Market Demand sub-score (`career-radar.ts`) joins `career_opportunities.company_or_org` against the same table so an opportunity at a known bank/telecom/mining employer counts as a target-industry match even when the opportunity's own title/category text doesn't literally mention that industry.

With Phase 8 shipped, all eight phases in `docs/CAREER_GROWTH_ENGINE_PLAN.md` §16 are complete. **Deferred to the plan's own §17 roadmap**, per this codebase's established "ship a real slice, document the rest" discipline: multi-source job discovery (scraping job boards/company career pages/government portals), real GitHub/LinkedIn activity integration (Career Radar's Visibility score is an honest recency proxy, not this), a salary negotiation AI coach, mock-interview voice/video mode, AI Auto-Pitch Customizer's "research the hiring manager" (needs web-search tool-calling this codebase doesn't have wired for any provider yet), learning-path generation tied to real course catalogs, confidence auto-tuning of match scores from outcome history, and — narrower, called out per-phase above — Modern/Executive resume template variants beyond the shipped ATS-plain default and a true AI-relevance Skills sub-score (vs. Phase 7's list-completeness proxy).

---

## Zuri CV Studio & Job Search OS

See `docs/CV_STUDIO_PLAN.md` for the full design — a deliberate course-correction on top of Career Growth Engine Phase 3's "describe your resume, AI writes it" flow, which is real but strategically wrong for job seekers: an AI that invents career content on a document a person then submits to a real employer is a genuine harm, not a demo weakness. **The philosophy changes from "AI writes your CV" to "Zuri helps you build the best version of your real professional history"** — every AI operation narrows to polish/rewrite/reorganise/suggest/analyse/score/tailor, never generate-from-nothing. Alongside this, Career OS's roadmap is reprioritized: **Job Search OS becomes the daily-engagement surface**, with the rest of Career OS (already shipped, see above) as supporting infrastructure underneath it. The CV Studio itself (Master Career Profile, the 14-step wizard, four templates, ATS Analysis/CV Health, tailored variants, the Web Editor) has not been built yet — only Job Search OS's Core Discovery Loop and Depth phases have shipped so far, per the plan's own §18 build order (the user's explicitly flagged top priority, independent of and ahead of the CV Studio foundation).

**Job Search OS — Core Discovery Loop** (see plan §15.1–§15.10, §18 Phase 2): reframes "job discovery" from a job board's question ("what jobs exist?") to an AI Opportunity Discovery Engine's question ("what jobs can this specific user realistically get today?"). `services/intelligence/app/services/job_discovery.py`'s `JobDiscoveryService` runs once per user per day: an **AI Search Planner** (`PLAN_JOB_SEARCHES`, `ai/prompts.py`) takes the user's full `career_profiles` row plus recent/rejected `career_opportunities` and returns a categorized set of real search-engine query strings — never one generic query — bucketed into five passes (local, regional, remote, freelance, "hidden" social/community-indexed phrasing), with Pass 1 (Local) reusing the already-seeded `career_employer_categories` (Career Growth Engine Phase 8) to generate per-known-employer queries when the user's target industries match a seeded category. Query volume is scaled by the AI Usage Tiers principle (see below) via a simple proxy — the user's subscription plan's `ai_replies_per_day` limit maps to 8/15/25 queries (light/normal/heavy) — since no dedicated per-user tier signal exists yet; regional/remote passes are additionally gated in code against `relocation_preference`/`remote_preference` as a safety net beyond the planner's own prompt instruction. Each query runs through the existing `web_search.py` (Tavily/SERP) — deliberately reused rather than any native provider search-grounding, the same judgment already made and documented for `interest_companion.py` in `docs/ADVISOR_COMPANION_PLAN.md`, since `ai/client.py` still has no grounding/tool-calling wiring for any provider. A cheap keyword pre-filter (profile skills/target roles against each result's title+snippet) runs before any AI call is spent, and total extractions are hard-capped per run — cost discipline, not just the tier proxy.

Each surviving result goes through one combined **extraction + fit-summary** AI call (`EXTRACT_JOB_LISTING`) — deliberately one call covering both concerns rather than two separate passes, the same "combine adjacent concerns into one structured call" judgment `CLASSIFY_ADVISOR_TURN` already made for intent+memory-suggestion. This is extraction from real search-result text, not content generation: salary/posted-date/company are `null` unless literally stated, and the "why this might suit you"/"potential concerns" pair is grounded only in the extracted fields versus the user's own stated preferences, never invented. No "estimated competition" figure is generated anywhere — there is zero real signal behind one. Everything downstream is plain code, no further AI calls: within-batch dedup (fuzzy title+company match, keeping the most complete candidate — same discipline `action_bundles.py`'s new-product/supplier dedup already established) runs before against-DB dedup; a freshness check drops anything with a literally-stated posting date over 30 days old and penalizes anything over 14; and a deterministic scorer finally writes the already-existing-but-previously-unused `career_opportunities.match_score`/`match_breakdown` columns (Career Growth Engine Phase 1, migration `0078`) — skills overlap, location/remote fit, salary fit, category fit, and freshness, each individually visible in the breakdown so the UI never has to trust a bare percentage. Matched opportunities insert with `source = 'web_search'`.

Once a day's run completes, a **Daily Opportunity Brief** — plain templated text, not an LLM call, grounded entirely in that run's literal counts ("I found 11 new opportunities. 4 are excellent matches. 3 are remote.") — delivers via the existing `companion_delivery.deliver_initiated_message()` (same mechanism `career_coach.py`/`motivational_detector.py` already use). Wired as a tenth daily scheduler at **05:00 UTC** (`daily_worker.py`/`main.py`) — the next free slot on an otherwise fully-booked 03:00–20:00 UTC schedule, deliberately chosen to land as a genuine Zambia morning (07:00 local) so the discovery run and the brief reporting on it happen together. Frontend: `opportunity-card.tsx` (Career Growth Engine Phase 4) gained a "why this job?" expandable checklist — tapping the match-score percentage reveals the per-dimension breakdown and a missing-skills line, rendered directly from `matchBreakdown` (never a separately-generated narrative that could drift from the actual score) and only enabled when a breakdown is actually present.

**Job Search OS — Depth** (see plan §15.11–§15.15, §18 Phase 3): `resume_studio.py` gained `match_opportunity_to_resumes()` — the inverse of Career Growth Engine Phase 3's `match_resume_to_opportunities()` (same cosine-similarity mechanism, opportunity→resumes instead of resume→opportunities); below a 70% best-match score it surfaces `suggestTailoring: true` rather than ever proposing invented experience to close the gap. **Application Readiness** (`GET /api/career/opportunities/:id/readiness`) is plain deterministic SQL — no AI call — checking for a resume, a cover letter, a portfolio/GitHub/LinkedIn URL, and listed certifications, same "exact thresholds over narrative" discipline as Studio's Zuri Insights. **Ghosting detection** deliberately doesn't duplicate Reality Engine's existing `contradiction_stalled_application` check (Career Growth Engine Phase 7 already flags an opportunity stuck in `applied` 21+ days) — the new piece is a company-scoped read in the new `services/intelligence/app/services/company_intelligence.py`: a plain SQL aggregation of the user's own past applications to that specific company, never a claim about the company's reputation generally. That same service also runs **Company Intelligence** proper — three `web_search.py` queries ("{company} culture reviews"/"recent news"/"interview process") through one synthesis call (`SYNTHESIZE_COMPANY_INTELLIGENCE`, `ai/prompts.py`) that explicitly declines any claim with zero search evidence behind it rather than inventing a plausible-sounding one, citing how many sources actually supported each populated field. `GET /api/career/opportunities/:id/company-intelligence` (Node) merges this with the existing interview-patterns lookup (Career Growth Engine Phase 4) into one response, so the frontend gets both without two separate calls. The **Passive Opportunity Radar's beyond-jobs extension** needed zero new schema — `PLAN_JOB_SEARCHES` gained a `beyond_jobs` query pass, gated in `job_discovery.py` on a `has_business_profile OR consulting-keyword-in-profile` signal, generating queries across the partnership/consulting/speaking/grant/tender/board_position/etc. category vocabulary Career Growth Engine Phase 1 already seeded. Frontend: `opportunity-card.tsx` grew three lazy-loaded expandable panels (Readiness, Resume match, Company intel) — extracted into a new `opportunity-insights.tsx` once the card file approached the codebase's 500-line File Architecture threshold.

**CV Studio Phase 1 — Master Career Profile foundation** (see plan §3, §18 Phase 1): migration `0081` adds nine per-entry tables (`career_employment_history`/`career_education_entries`/`career_certifications`/`career_skill_groups`/`career_awards`/`career_volunteer_work`/`career_memberships`/`career_publications`/`career_references`) upgrading Career Growth Engine Phase 1's flat JSONB-array `career_profiles` fields into real, field-by-field-editable rows, plus the CV-as-object-model tables (`career_cvs`/`career_cv_sections`/`career_cv_project_links`/`career_cv_versions`) and `career_profiles`' Step 1/Step 14 wizard columns (`phone`/`location`/`website_url`/`driving_licence`/`nationality`/`passport_or_nrc`/`availability`/`notice_period`/`interests`/`references_mode`/`default_page_size`/`use_cv_terminology` — willing-to-relocate/expected-salary deliberately reuse the existing `relocation_preference`/`salary_expectation_cents` fields rather than duplicating them). A real backfill migrates existing `career_profiles.skills`/`certifications`/`education` JSONB data into the new tables rather than starting every existing user at zero. `projects.is_portfolio_visible` (default `true`) is the one column addition outside `career_*` — the flag that lets a project be excluded from the CV's Projects-section picker. `services/api/src/lib/career-entry-crud.ts`'s `registerCareerEntryRoutes()` is a shared factory (list/create/patch/delete against a small column config) mounting all nine entry resources from `career-profile-entries.ts` — nine hand-written near-identical CRUD blocks would have been pure duplication; each resource still keeps its own explicit Zod schema. `career-cvs.ts` is the CV object model's own file: create/patch/delete, whole-list-replace for sections and project-links (the same "a template is edited as one ordered set" convention Services Management's workflow-stage PUT already established), duplicate (a new row with `source_cv_id` set, copying section config), and version history — every `structuredContent` PATCH writes a new `career_cv_versions` row and restore copies an old snapshot forward, never destructively, so later phases (the Web Editor, Version History UI) inherit working version data for free. **Deliberately deferred**: the plan's own §18 Phase 1 scope also calls for deleting `resume_studio.py`'s whole-document generation functions (`generate_resume_data`/`generate_cover_letter_data`) — not done here, since they're still the only working resume-generation path in production (Career Growth Engine Phase 3's shipped Resume Studio UI calls them directly) and deleting them now would regress a live feature with no replacement; that deletion waits for Phase 4/5's real wizard render pipeline.

**CV Studio Phase 4 — The Wizard** (see plan §4, §18 Phase 4): 14 autosave-on-blur steps over Phase 1's tables, no "Save"/"Generate" button anywhere in the flow. Eight steps (Employment, Education, Certifications, Skills, Awards, Volunteer Work, Memberships, Publications) share one generic `EntryListEditor` component (`career/_components/cv-studio/entry-list-editor.tsx`) driven by a small field-config array per step — the same DRY discipline `career-entry-crud.ts`'s backend factory already established, carried into the frontend rather than eight near-identical hand-written forms. The remaining six (Personal Details, Summary, Objectives, Projects, References, Additional Info) are bespoke: Projects is a checkbox-picker over the user's own `projects` rows writing `career_cv_project_links` via the whole-list-replace endpoint; References toggles between "available on request" and a listed `EntryListEditor` instance; Personal Details/Summary/Objectives/Additional Info share a `useCareerProfile` hook (one fetch, optimistic-update-then-PATCH per field). A new rewrite-only AI Assistant (`services/intelligence/app/services/cv_assistant.py`, prompts `REWRITE_CV_TEXT`/`SUGGEST_METRIC_PROMPT`/`SUGGEST_SKILL_GROUPING`, all prefixed with `CV_STUDIO_NEVER_INVENT_POLICY`) powers Summary/Objectives/Employment's AI buttons — Improve/Shorten/tone variants/ATS-optimise/fix-grammar/responsibilities-to-achievements — plus "Add Metrics," which asks a clarifying question rather than inventing a number, per the plan's own worked example. Live preview (`cv-preview.tsx`) is a lightweight HTML/CSS mirror of the Professional template's layout, not a PDF round-trip per keystroke: `career_profiles` fields update instantly from already-live local state, while the nine per-entry-table sections refresh right after each save via a `refreshKey` bump — an honest, documented simplification of true per-keystroke reactivity across nine separate tables, not the real PDF render pipeline (that's Phase 5+). New routes: `/career/cv-studio` (list existing CVs, build a new Master CV) and `/career/cv-studio/[id]` (the wizard itself, linked from a new "CV Studio" button on `/career`'s hero card). **Not built in this pass**: skill-group AI-suggested-grouping has a working endpoint but no wizard UI trigger yet — a small polish item, not a core-loop gap.

**CV Studio Phase 5 — Templates + Render Pipeline** (see plan §5, §18 Phase 5): three new React-PDF templates — `CvModern.tsx` (indigo accent, clean single-column), `CvExecutive.tsx` (dark section bars, larger summary block, achievement-forward), `CvCreative.tsx` (two-column with a colored sidebar for contact/skills — the one template where visual impression deliberately outweighs ATS-parseability, per the plan's own scoping) — alongside the existing Professional (`Resume.tsx`, which gained an optional `pageSize` prop so all four templates now respect `career_cvs.page_size`/`career_profiles.default_page_size` consistently). A new `services/api/src/lib/pdf/cv-context.ts`'s `buildCvRenderData()` assembles a CV's live content directly from the Phase 1 relational tables (`career_profiles` + all nine entry tables + `career_cv_project_links` joined with `projects`) — a pure data read, no AI call, no round-trip to the intelligence service since Node already owns every table involved. `render.ts`'s `renderCvPdf()` picks the template component by `career_cvs.template_key`. `GET /api/career/cvs/:id/pdf` renders fresh on every request and streams the PDF directly — deliberately *not* persisted as a `documents` row (unlike the older whole-document Resume Studio flow this coexists with), matching the plan's own §9 framing that "the real render happens on-demand for download." The wizard (`wizard-shell.tsx`) gained a template-picker dropdown (PATCHes `templateKey` immediately) and a Download PDF button (blob-fetch-with-auth-header, same pattern `/business`'s document downloads already use) — the live preview itself intentionally stays the same plain HTML mirror regardless of which template is selected; only the downloaded PDF actually looks different per template.

**CV Studio Phase 6 — ATS Analysis + CV Health** (see plan §7, §18 Phase 6): ATS Analysis (`POST /api/career/cvs/:id/ats-score`) reuses `score_resume_text()`'s existing `SCORE_RESUME` prompt verbatim via the already-built `/internal/career/resume/score` intelligence endpoint — no new AI prompt was needed, only a new `buildCvPlainText()` helper (`services/api/src/lib/cv-health.ts`) serializing the CV's live relational data into the plain text that prompt already expects. CV Health (`GET /api/career/cvs/:id/health`) is a fully deterministic scorer with zero AI calls — `computeCvHealth()` checks missing LinkedIn URL, no quantified achievements (a regex for digit/%/currency across achievement text), a Professional Summary that's missing/too short/too long, an unexplained 90+ day gap between two employment entries, an estimated-too-long CV (a line-count heuristic), zero certifications, and weak-verb phrasing ("responsible for", "worked on") — each deducting fixed points from a 100 baseline and carrying its own one-tap fix-action description, the same "exact thresholds beat narrative" discipline Studio's Zuri Insights already established. Frontend: a new `CvHealthPanel` renders below the wizard's live preview — CV Health loads automatically since it's cheap; ATS Analysis is a button-triggered on-demand call (it's a real AI cost) showing all five `SCORE_RESUME` sub-scores plus concrete rewrite suggestions.

**CV Studio Phase 7 — Web Editor + Version History** (see plan §9, §10, §18 Phase 7): a distinct post-wizard page (`career/cv-studio/[id]/editor/page.tsx`, reached via a new "Editor" button in the wizard header) rather than a 15th wizard step — the plan itself frames the Web Editor as a separate editing mode over an already-populated CV, not another stop in the linear content-entry flow. `web-editor-panel.tsx`'s `WebEditorPanel` lets a user reorder, hide, and rename `career_cv_sections` against the exact `PUT /api/career/cvs/:id/sections` whole-list-replace endpoint Phase 1 already shipped — no backend changes needed for this panel at all. Reorder is up/down arrow buttons, not drag-and-drop: this codebase has no drag/sortable library anywhere yet, and pulling one in solely for one panel isn't worth the new dependency — arrows are a fully working, keyboard-accessible mechanism that needs none. `version-history-panel.tsx`'s `VersionHistoryPanel` lists Phase 1's `GET /api/career/cvs/:id/versions`, restores a version via the existing non-destructive copy-forward endpoint, and duplicates the whole CV via the existing `POST .../duplicate`, navigating straight into the new copy's wizard. "Compare" is a deliberate simplification of the plan's fuller vision: a plain side-by-side JSON snapshot display for two checkbox-selected versions, no line-level diff highlighting — an honest scope reduction, matching this codebase's "ship a real slice, document the rest" discipline, not a real diff algorithm built for one panel. The editor page still renders the live `CvPreview` alongside both panels, per the plan's own "still live-preview-backed" requirement for this mode.

**CV Studio Phase 8 — Tailored Variants + Job Matching UI** (see plan §8, §11, §18 Phase 8): "Create Variant" needed no backend work at all — `POST /api/career/cvs` has taken `sourceCvId`/`careerOpportunityId` since Phase 1's CRUD scaffolding — only a frontend entry point. `CvTailoringPanel` (new file, `career/_components/cv-tailoring-panel.tsx`, a sibling to `opportunity-insights.tsx` rather than another expandable panel piled into `opportunity-card.tsx`, which was already close to the File Architecture 500-line threshold) creates a variant from the user's master CV with `careerOpportunityId` set and navigates straight into its wizard, rendered alongside the existing Readiness/Resume-match/Company-intel panels on each opportunity card. Job Matching (§11) is a new intelligence service, `cv_matching.py`: `compute_cv_opportunity_match()` embeds the CV's live text — assembled by Node's existing `buildCvRenderData()`/`buildCvPlainText()`, the same functions CV Health (Phase 6) already uses, so the CV-assembly logic isn't duplicated in Python — against the opportunity's description using the exact cosine-similarity mechanism `resume_studio.py`'s `match_resume_to_opportunities()` already established for the older whole-document Resume Studio flow, plus one structured extraction call (`EXTRACT_JOB_REQUIREMENTS`) pulling only the skills/technologies a job posting's own text actually states, diffed against the CV's skill groups to produce a missing-skills list. `generate_tailoring_suggestions()` is the plan's own named "Generate Improvement Suggestions" action (explicitly never "Generate Fake Experience"): one `SUGGEST_CV_TAILORING` call, prefixed with the same `CV_STUDIO_NEVER_INVENT_POLICY` every rewrite call already carries, proposing which of the CV's *existing* bullets/skills to emphasise or reorder for this specific opportunity — grounded only in text already present in the CV, never inventing a new claim to close a gap. `services/api/src/routes/career-cv-matching.ts` (a new sibling file to `career-cvs.ts`, which was already ~400 lines) proxies both new endpoints — `GET /api/career/opportunities/:id/match/:cvId` and `GET .../tailoring-suggestions?cvId=` — assembling the CV text/skills itself so the intelligence service never needs to query `career_cvs`/the entry tables directly.

**CV Studio Phase 9 — Cover Letter Studio + Supporting Documents** (see plan §12, §13, §18 Phase 9 — the final CV Studio phase): migration `0082` widens `documents.document_type`'s CHECK with `application_letter`/`expression_of_interest`/`personal_statement`/`motivation_letter`/`reference_sheet`/`portfolio_pdf`, the same widen precedent every prior document-type addition already used. Cover Letter Studio is deliberately **not** a second invent-from-instruction generator — the pre-existing `/api/career/cover-letter` (Career Growth Engine Phase 3) is left exactly as-is, since Resume Studio's own "Generate Cover Letter" button still calls it and there's no forcing function to regress a live feature over. Instead, a real **compose-and-save** path is what this phase ships: `resume_studio.py`'s `create_resume_document()` gained a generic `_DOCUMENT_TYPE_LABELS` map (previously hardcoded to two types) and one new shared `/internal/career/documents/save` route accepting any of the six new types plus `cover_letter` — a single route rather than six near-identical ones, since only `structured_data` differs and Node already assembles it. `career-documents.ts` gained `POST /api/career/letters/compose` (recipientName/companyName/body/signOff — the body is real, user-composed text, optionally AI-polished via the pre-existing generic `/api/career/cv-assistant/rewrite` endpoint, never generated from a free-text instruction the way the old flow works), `POST /api/career/reference-sheet` (a plain, non-AI rendered view of `career_references`), and `POST /api/career/portfolio-pdf` (a plain, non-AI rendered view of `projects.is_portfolio_visible = true` rows) — the latter two need zero user input beyond an optional title. Two new React-PDF templates, `ReferenceSheet.tsx`/`PortfolioPdf.tsx`; the four letter-shaped Phase 9 types reuse `renderCoverLetterPdf()` verbatim (identical `{recipientName, companyName, body, signOff}` shape) rather than a duplicate template — `document-render.ts`'s career-document branch (previously `resume`/`cover_letter` only) now covers all eight types. Frontend: a new `CoverLetterStudio` component (`career/_components/cover-letter-studio.tsx`, its own file per the File Architecture convention — `resume-studio.tsx` was already ~300 lines) renders alongside Resume Studio on `/career`: "Compose a Letter" opens a modal listing the user's own real employment achievements as one-tap-to-insert chips (plain data, no AI) above a body textarea wired to the existing `AiRewriteToolbar` for polish-only rewriting; "Reference Sheet"/"Portfolio PDF" are one-click generate buttons. The plan's own "Zuri suggests surfacing achievements for a specific opportunity" is deliberately *not* wired into this modal as a separate AI call — Phase 8's `generate_tailoring_suggestions()` already does exactly this, but scoped to a `career_cvs` row via `cvId`, which a from-scratch letter compose doesn't have; forcing that fit would have meant either a fake CV requirement or a second, parallel suggestion mechanism, so this phase ships the achievement-picker (real, deterministic, always available) and leaves "AI-ranked which achievement to lead with" as a documented follow-on once a compose-time CV context is worth requiring.

With Phase 9 shipped, all nine phases of `docs/CV_STUDIO_PLAN.md` §18 are complete.
...
## File Architecture — Avoiding Monolithic Files

This codebase has a real, measured problem: `apps/web/src/app/(dashboard)/studio/page.tsx` is ~5,000 lines (a dozen-plus module components, each with their own state/effects/handlers, all defined inline in one file), and several other files — `contacts/[id]/page.tsx`, `settings/page.tsx`, `services/api/src/routes/analytics.ts`, `inbox/_components/intel-panel.tsx`, `business/page.tsx` — are 1,000–2,500 lines. This happened because each new module/tab/route was added as "just one more component in the same file" rather than a new file. **From now on, new work must not add to that problem, and should chip away at it where a file is touched anyway.**

**Frontend — one module/tab component per file.** A dashboard page like `studio/page.tsx` should be a thin shell: fetch the page-level `MODULES` config, render the tab bar, and `import` each module (`OverviewModule`, `CatalogModule`, `ServicesModule`, `InventoryModule`, etc.) from its own file under a co-located `_components/` directory (the pattern `inbox/_components/` already establishes — e.g. `studio/_components/services-module.tsx`, `studio/_components/catalog-module.tsx`). Sub-components used by only one module (a card, a modal, a panel) live either inlined in that module's file if small, or in their own file under the same `_components/` directory once the module file itself starts exceeding ~400–500 lines. Shared, cross-module pieces (badge helpers, formatters, small presentational atoms) belong in `apps/web/src/lib/` or `apps/web/src/components/ui/`, not duplicated per module.

**Threshold, not a hard rule**: a single `.tsx`/`.ts`/`.py` file crossing roughly **500 lines** is the signal to stop and ask "does this file now hold more than one cohesive concern?" — not an automatic split. A route file that's 700 lines of one resource's straightforward CRUD (e.g. `products.ts`) is fine; a file that's 700 lines because it's grown a second unrelated resource inside it is not. When in doubt, prefer the split this repo already uses elsewhere: `services.ts` was born as a sibling to `products.ts` specifically so pricing-tiers/capacity/workflow-stages logic didn't pile into the products route file — that's the model to repeat, not the exception.

**API routes (Fastify, `services/api/src/routes/*.ts`)**: one file per resource/domain, exactly as already practiced (`products.ts`, `documents.ts`, `projects.ts`, `services.ts`, `goal-profiles.ts`, etc.). When a resource grows a genuinely separate sub-concern (the way Services grew out of Products, or Purchase Orders grew out of Documents), give it its own route file and import shared helpers (`computeTotals`, `assignDocumentNumber`, `formatDocument`, `lineItemSchema`) from the parent file rather than duplicating them — `projects.ts`'s `POST /api/projects/:id/documents` endpoint importing from `documents.ts` is the reference example. `analytics.ts` at 2,000+ lines is the one file that's overdue for a split (by dashboard section — sales/customers/operations/predictions/etc. each as their own route file registered alongside it) whenever it's next touched for a non-trivial change; don't add more to it as-is.

**Intelligence service (`services/intelligence/app/services/*.py`)**: one class per file, one responsibility per class — already the convention (`action_bundles.py`, `document_followups.py`, `project_progress.py`, `pricing_benchmarks.py` are each under 150 lines and do exactly one thing). Keep new detectors/schedulers/services this small rather than growing an existing file to cover a second concern — a new nightly check gets a new file plus a new scheduler entry in `daily_worker.py`, not a new method bolted onto an unrelated existing service.

**Large pre-existing files are not blocking** — `studio/page.tsx` doesn't need a big-bang refactor before new Studio work can happen. But any task that touches it for a substantial addition (a new module, a large new form) should extract that new piece into its own file under `studio/_components/` rather than appending to the existing 5,000-line file, and should extract the specific module being modified if the edit already requires touching a large contiguous block of it. The goal is that the file shrinks by attrition as it's touched, not that it's frozen in its current state.
