...
**Deferred to `docs/CAREER_GROWTH_ENGINE_PLAN.md`'s later phases**, per this codebase's established "ship a real slice, document the rest" discipline: Modern/Executive resume template variants beyond the shipped ATS-plain default, Intelligent Job Discovery with Zambia-first localisation, the AI Application Engine, mock-interview coaching, match scoring on `career_opportunities.match_score`/`match_breakdown` themselves (Phase 3 scores resumes and matches them to opportunities, but doesn't yet write a per-opportunity match score back onto the opportunity row), the Relationship-to-Opportunity Bridge ("shortest path" introduction suggestions), Career Radar (the 0–100 opportunity-readiness composite score), and Career Coach/Motivation & Accountability — none built in Phases 1–4.
