    ...
    <div className="bg-[linear-gradient(180deg,#eef2ff_0%,#f8fafc_260px,#f8fafc_100%)]">
      <div className="p-4 md:p-6">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="rounded-[1.75rem] border border-gray-100 bg-white shadow-sm shadow-gray-200/70 p-4 sm:p-5">
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    <h1 className="text-base sm:text-lg font-bold text-gray-950">{project.title}</h1>
                    {project.careerOpportunityId && (
                      <Link href={`/career/jobs/${project.careerOpportunityId}`} className="inline-flex items-center gap-1 rounded-full bg-indigo-50 px-2.5 py-1 text-xs font-semibold text-indigo-700 hover:bg-indigo-100 ring-1 ring-indigo-200">
                        <Briefcase className="w-3.5 h-3.5" /> Job Application
                      </Link>
                    )}
                  </div>
                  <p className="text-xs text-gray-500">
                    {project.contactName ?? 'No contact linked'}
            ...
            <div className="flex flex-wrap items-center gap-2 mt-4 pt-3 border-t border-gray-100/80">
              <div className="grid grid-cols-2 sm:flex sm:flex-wrap items-center gap-1.5 w-full sm:w-auto">
                {(['active', 'on_hold', 'completed', 'cancelled'] as const).map(s => (
                  <button
                    ...
                    className={`min-h-[44px] sm:min-h-[36px] px-3 py-2 sm:py-1.5 rounded-xl text-xs font-bold transition-all disabled:cursor-default ${
              ...
              <div className="flex items-center gap-3 ml-auto pt-2 sm:pt-0 w-full sm:w-auto justify-between sm:justify-end border-t sm:border-t-0 border-gray-100">
                <button
                  onClick={openGoalPicker}
                  className="min-h-[44px] sm:min-h-0 inline-flex items-center gap-1.5 text-xs font-semibold text-indigo-600 hover:text-indigo-700 transition-colors"
                >
                ...
                <button
                  ...
                  disabled={deleting}
                  className="min-h-[44px] sm:min-h-0 inline-flex items-center gap-1.5 text-xs font-semibold text-red-500 hover:text-red-700 transition-colors disabled:opacity-50"
                >
          ...
          <div className="rounded-[1.75rem] border border-gray-100 bg-white shadow-sm shadow-gray-200/70 mt-4">
                ...
                return (
                  <div key={task.id} className="flex items-center gap-3 px-4 py-3.5 hover:bg-gray-50/80">
                    ...
                    <button
                      ...
                      disabled={busyTaskId === task.id}
                      className="text-gray-400 hover:text-indigo-600 transition-colors flex-shrink-0 min-w-[40px] min-h-[44px] flex items-center justify-center"
                      title="Edit task"
                    >
                    ...
                    <button
                      ...
                      disabled={busyTaskId === task.id}
                      className="text-gray-400 hover:text-red-500 transition-colors flex-shrink-0 min-w-[40px] min-h-[44px] flex items-center justify-center"
                      title="Delete task"
                    >
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 px-4 py-3.5 border-t border-gray-50">
              <input
                value={newTaskTitle}
                onChange={e => setNewTaskTitle(e.target.value)}
                onKeyDown={e => { if (e.key === 'Enter') addTask() }}
                placeholder="Add a task…"
                className="flex-1 text-sm border border-gray-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 min-h-[44px]"
              />
              <button
                onClick={addTask}
                disabled={addingTask || !newTaskTitle.trim()}
                className="inline-flex items-center justify-center gap-1.5 text-xs font-bold px-4 py-2.5 bg-indigo-600 text-white rounded-xl hover:bg-indigo-500 disabled:opacity-50 transition-colors min-h-[44px]"
              >
