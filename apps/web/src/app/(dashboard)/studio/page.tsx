...
interface StudioInsights {
  ...
  recentEvents: {
    id: string; eventType: string; confidence: number | null; evidence: string[]
    payload: Record<string, any>; status: string; bundleId: string | null
    contactName: string | null; createdAt: string
  }[]
}
  ...
  async function handleChatAction(action: ParsedAction) {
    ...
    switch (action.type) {
      ...
      case 'estimate_duration': {
        ...
        const { stages } = await apiClient<{ stages: { name: string }[] }>(`/api/products/${productId}/workflow-stages`, { token })
        if (!stages.length) {
          return 'No workflow template set for this service yet — add stages in the Services tab to get a duration estimate.'
        }
        return `This service's workflow has ${stages.length} stage${stages.length === 1 ? '' : 's'}: ${stages.map(s => s.name).join(', ')}. Add typical durations per stage in the Services tab for a precise timeline.`
      }
      case 'start_project': {
        const [productId, contactId] = action.params
        return apiClient<{ projectId: string; title: string; taskCount: number }>(
          `/api/products/${productId}/start-project`, { method: 'POST', token, body: JSON.stringify({ contactId: contactId || undefined }) },
        )
      }
    }
  }
