...
END:VCARD`
  ...
  if (!data) {
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
        <div className="max-w-md text-center space-y-4 bg-slate-800/80 border border-slate-700 p-8 rounded-3xl text-white">
  ...
  const { profile, employment, education, skillGroups, projects } = data
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 relative z-10 space-y-8">
        ...
        <header className="flex items-center justify-between pb-4 border-b border-slate-800/80">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-extrabold tracking-tight text-white">Zuri Living Portfolio</span>
          </div>
        ...
        {/* Hero Card */}
        <div className="relative overflow-hidden rounded-3xl border border-slate-800 bg-slate-900/90 backdrop-blur-xl p-6 sm:p-10 shadow-2xl space-y-6">
          <div className="absolute top-0 inset-x-0 h-1 bg-gradient-to-r from-amber-400 via-emerald-400 to-indigo-500" />

          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div className="flex items-center gap-5">
              <div className="relative">
                <div className="flex h-20 w-20 sm:h-24 sm:w-24 items-center justify-center rounded-2xl bg-gradient-to-br from-indigo-600 to-slate-800 text-white text-2xl sm:text-3xl font-black border-2 border-amber-400/30 shadow-xl">
                  {initials}
                </div>
                <div className="absolute -bottom-1 -right-1 flex h-6 w-6 items-center justify-center rounded-full bg-emerald-500 text-slate-950 border-2 border-slate-900" title="Verified Professional">
              ...
              <div>
                ...
                <p className="text-sm sm:text-base font-bold text-amber-400 mt-1">{profile.headline}</p>
                <div className="flex flex-wrap items-center gap-3 text-xs text-slate-400 mt-2">
                  <span className="flex items-center gap-1"><MapPin className="w-3.5 h-3.5 text-indigo-400" />{profile.location}</span>
                  <span>•</span>
                  <span className="flex items-center gap-1"><UserCheck className="w-3.5 h-3.5 text-emerald-400" />Verified Portfolio</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex-1 md:flex-initial inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold px-4 py-2.5 text-xs shadow-md transition-all active:scale-95"
              >
              ...
              <a
                ...
                className="flex-1 md:flex-initial inline-flex items-center justify-center gap-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-white font-bold px-4 py-2.5 text-xs border border-slate-700 transition-all"
              >
              ...
              <button
                ...
                className="inline-flex items-center justify-center p-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 transition-all"
          ...
          <div className="flex flex-wrap items-center gap-3 pt-4 border-t border-slate-800/80 text-xs">
            {profile.githubUrl && (
              <a href={profile.githubUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-slate-400 hover:text-white transition-colors">
                <GithubIcon className="w-3.5 h-3.5" />
                <span>GitHub</span>
              </a>
            )}
            {profile.linkedinUrl && (
              <a href={profile.linkedinUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-slate-400 hover:text-white transition-colors">
                <LinkedinIcon className="w-3.5 h-3.5 text-sky-400" />
                <span>LinkedIn</span>
              </a>
            )}
            {profile.phone && (
              <a href={`tel:${profile.phone}`} className="inline-flex items-center gap-1.5 text-slate-400 hover:text-white transition-colors">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <section className="rounded-3xl border border-slate-800 bg-slate-900/60 p-6 space-y-3">
              ...
              <p className="text-sm text-slate-300 leading-relaxed font-normal">{profile.summary}</p>
            </section>
              ...
              <div className="space-y-6">
                {employment.map((e) => (
                  <div key={e.id} className="relative pl-5 border-l-2 border-indigo-500/30 space-y-2">
                    <div className="absolute -left-[9px] top-0 h-4 w-4 rounded-full bg-slate-900 border-2 border-indigo-400" />
                    ...
                    <p className="text-xs font-semibold text-slate-400">{e.employer}</p>
            ...
            {projects.length > 0 && (
              <section className="rounded-3xl border border-slate-800 bg-slate-900/60 p-6 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {projects.map((p) => (
                    <div key={p.projectId} className="p-4 rounded-2xl bg-slate-950/60 border border-slate-800 space-y-1.5">
          ...
          <div className="space-y-8">
              ...
              <div className="space-y-3">
                {skillGroups.map((g) => (
                  <div key={g.id} className="space-y-1.5">
                    <span className="text-xs font-bold text-slate-300">{g.groupName}</span>
                    <div className="flex flex-wrap gap-1.5">
                      {g.skills.map((skill, idx) => (
                        <span key={idx} className="px-2.5 py-1 rounded-xl bg-slate-800 border border-slate-700 text-[11px] font-semibold text-slate-200">
                          {skill}
                        </span>
                      ))}
                    </div>
                ...
                <div className="space-y-3 text-xs">
                  {education.map((e) => (
                    <div key={e.id} className="space-y-0.5">
                      <p className="font-bold text-white">{e.qualification}</p>
                      <p className="text-slate-400">{e.institution} ({e.endDate})</p>
                    </div>
                  ))}
                </div>
              </section>
            ...
            <section className="rounded-3xl border border-amber-500/30 bg-gradient-to-br from-slate-900 via-slate-900 to-amber-950/20 p-6 space-y-4 shadow-xl">
              <div className="flex items-center gap-2">
                <Send className="w-4 h-4 text-amber-400" />
                <h2 className="text-sm font-bold text-white">Contact Candidate</h2>
              </div>
              <p className="text-xs text-slate-400">Direct recruiter intake. Delivered directly to {profile.fullName}.</p>

              {inquirySent ? (
                <div className="p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-xs text-emerald-400 text-center font-bold">
        ...
        <footer className="pt-8 border-t border-slate-800/80 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-500">
          ...
          <p>Powered by <strong className="text-slate-300">Zuri AI Relationship OS</strong></p>
        </footer>
      </div>
