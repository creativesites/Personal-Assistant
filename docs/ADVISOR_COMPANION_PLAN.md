...
**✅ Shipped** (migration `0066`). See `CLAUDE.md`'s "Advisor Companion — Phase 0" section for the full summary, including two corrections against this plan's original assumptions (below).
...
- ~~Reconsolidation (§6.8) runs platform-wide already~~ — **correction:** it did not. Neural Layer Phase 1 shipped `emotional_signals.memory_weight` but nothing ever wrote to it; Phase 0 adds the nightly reconsolidation job that was actually missing (see scope note below).
...
**Scope notes / corrections against the plan as originally written:**

1. **`advisor_user_profiles` (§4.1) had to be created here, a phase early.** The plan assigns the base table to Phase 1 (§9) and only its extension columns (§4.5) to Phase 0 — but Phase 0's own columns have nowhere to live without the table existing. Migration `0066` creates the full base table (§4.1: `display_persona`/`tone_preferences`/`advice_preferences`/`boundaries`/`relationship_context`/`learned_traits`/`confidence`/`last_refined_at`) plus the Phase-0-scoped subset of §4.5 (`interests`, `spiritual_preferences`, `motivational_style`, `gossip_style`, `current_emotional_state`, `emotional_baseline`, `companion_features_paused`). It deliberately does **not** add `personal_mode_enabled`/`personal_mode_enabled_at` — the plan explicitly assigns those two to Phase 1, and Phase 1 will add them via its own migration when it starts, plus build the profile/memory routes on top of the table that already exists rather than creating it again.
2. **Wiring Advisor turns to `emotional_signals` was already done** — Neural Layer Phase 1 already calls `get_emotion_engine().record_advisor_turn(...)` from both `/internal/advisor/ask` and `/internal/conversations/:id/ask` in `services/intelligence/app/routes/conversation.py`. Phase 0 adds one more piece on top: `record_advisor_turn` now also upserts `advisor_user_profiles.current_emotional_state` after every write (§6.6's third bullet), which didn't exist until this phase since the table itself didn't exist.
3. **State-dependent retrieval weighting (§6.7)** ships as `emotional_congruence()` in `neural/emotion.py` — a pure function (`0..1`, blends into an existing score rather than being a separate retrieval path) — wired into the one concrete ranked-candidate list Advisor already builds today: the global advisor's (`/internal/advisor/ask`) "recent contacts" context, previously ordered by recency only. It's now reranked by `0.7 × recency + 0.3 × emotional congruence` with the user's `current_emotional_state`, using each contact's trailing-14-day average valence/arousal from `emotional_signals`. `advisor_memories` (Phase 1) and the associative emotional graph (Phase 5) will be additional consumers of the same function once those tables/features exist — this isn't a bespoke one-off scorer.
4. **Reconsolidation (§6.8) is newly built here**, in `neural/emotion.py`'s `EmotionEngine.reconsolidate()`, scheduled nightly at 04:00 UTC (`run_emotion_reconsolidation_scheduler` in `daily_worker.py`, after the existing 03:00 consolidation job). Simplified vs. the plan's own `last_retrieved_at`-tracked design — no retrieval call site marks rows as "accessed" yet, so it revisits every `emotional_signals` row from the trailing 24h instead of only ones actually read that day, and pulls a negative-valence row's `memory_weight` down when that same user+contact pair's more recent (trailing 3-day) average valence has since improved materially — the plan's own "a resolved fight matters less now" example, just triggered by elapsed time rather than a real access event. Also recomputes `advisor_user_profiles.emotional_baseline` as a rolling 30-day average across a user's `advisor_turn`-scoped signals.
5. **Incidental fix:** while wiring this phase's scheduler into `services/intelligence/app/main.py`, found that Neural Layer Phase 3's `create_reflection_worker`/`run_reflection_scheduler` were imported but never actually instantiated or started in the lifespan function — a latent bug from that phase (the weekly reflection job has never run in production). Fixed alongside this phase's own worker registration.

### Phase 1 — Companion Brain Foundation

**✅ Shipped** (migration `0067`). See `CLAUDE.md`'s "Advisor Companion" section for the full summary and scope notes below.
...
1. **Scoped to the global advisor only.** `AdvisorCompanionService.handle_turn()` (`services/intelligence/app/services/advisor_companion.py`) owns the full turn for `/internal/advisor/ask` — intent + memory-suggestion classification, profile/memory/emotional-state retrieval, dynamic prompt assembly, the Phase 0 contact-congruence reranking (moved here from `routes/conversation.py`), and calling the model. The conversation-scoped (`/internal/conversations/:id/ask`) and Studio (`/internal/studio/ask`) advisors keep their existing, separate flow — folding them into this same orchestrator, along with the deeper per-conversation retrieval §3.3 describes, is Phase 2 scope ("scoped contact/conversation retrieval for Advisor").
2. **Intent + memory suggestion in one call.** `CLASSIFY_ADVISOR_TURN` (`ai/prompts.py`) combines §6.2's intent classifier and a light version of §6.5's memory learner into a single structured completion, rather than two separate LLM calls per turn. `memory_suggestion` is null on most turns — it only fires when the model judges the user stated something durable (a preference, boundary, trait, goal, or reaction to advice). A proposed memory is inserted directly into `advisor_memories` (not held for a separate confirm step) since the memory drawer already makes it visible/editable/forgettable — the "Should I remember this?" confirmation card (§7.1) is a nice-to-have UI polish, not required for the "transparent and editable" bar §3.4 sets.
3. **Companion mode chips are mechanical in Phase 1, not fully realized.** `advisor_sessions.companion_mode` is settable via chip or `PATCH /api/advisor/sessions/:id`, and shifts the system prompt's framing (a per-mode instruction line in `_COMPANION_MODE_INSTRUCTIONS`) — this is genuinely "Advisor changes tone in-session" (the success criterion), but `gossip` and `spiritual_companion` are only basic tone shifts here, not the grounded-evidence gossip experience (§3.7, Phase 2's own "reachability" bullet) or the devotional/verse content (§3.9, Phase 4.5). The `spiritual_companion` chip only renders once `spiritual_preferences.tradition` is set, honoring the consent gate from day one even at this mechanical level.
4. **Memory drawer and Personalisation tab are real, but minimal.** Both live inside `/advisor`'s existing right inspector panel as new tabs (Memory, Personalize) alongside the existing Context tab — no separate page, no onboarding surface, matching §7.6's explicit "discoverability, not disclosure" instruction. The Memory tab supports add/forget (soft-delete via `is_active`) but not yet the full "That's wrong" correction UX in the chat itself (`POST /api/advisor/memories/:id/correct` exists as an API but has no chat-inline entry point yet — only reachable from the drawer for now).
5. **`personal_mode_enabled`/`personal_mode_enabled_at` ship on `advisor_user_profiles`** exactly as planned, settable via the `activate_personal_mode`/`deactivate_personal_mode` intents (handled directly in `handle_turn()`, short-circuiting before any prompt assembly) or the Personalisation tab's toggle — never inferred.

### Phase 2 — Relationship Analysis Experience

**✅ Shipped** (no new migration). See `CLAUDE.md`'s "Advisor Companion" section for the full summary.
...
1. **The conversation-scoped advisor is now folded into `AdvisorCompanionService`**, exactly as Phase 1's own scope note promised. `handle_conversation_turn()` retrieves the transcript, contact profile, relationship memory (`retrieval_service.get_contact_summary`/`get_relationship_memory` — genuinely new retrieval for this endpoint; the pre-Phase-2 `ask_ai` only had the raw transcript and a few CRM fields), and the contact's/user's emotional state. `routes/conversation.py`'s `/internal/conversations/:id/ask` is now a thin delegator, same pattern as `/internal/advisor/ask` in Phase 1. `_get_recent_messages`/`_format_transcript` moved to `memory/retrieval_service.py` (`get_recent_messages`/`format_transcript`) so both the service and the still-separate `summarize`/`followup` endpoints share one implementation instead of two copies.
2. **Evidence/read/action structure only fires for analysis-flavored intents** (`chat_analysis`, `relationship_advice`, `emotional_support`) — `ANALYZE_CHAT_TURN` (`ai/prompts.py`) asks for one structured JSON completion combining the natural-language answer with `evidence`/`myRead`/`alternativeRead`/`whatIWouldDo`. Every other intent (draft_reply, casual_chat, business_analysis, etc.) uses `CONVERSATION_TURN`'s plain-text path — not every turn is "analysis," so not every turn pays for the structured contract or shows the evidence card.
3. **Relationship advice policy (§8.2)** is a static block (`RELATIONSHIP_ADVICE_POLICY`) folded into both prompts — never claim certainty about someone else's feelings, never fabricate evidence, don't encourage manipulation, don't present as a therapist, suggest real help for abuse/self-harm signals.
4. **Emotional context folding** is a simple threshold check, not a new signal: if the user's `current_emotional_state` shows valence below -0.2 and arousal above 0.6, the prompt gets one extra line asking the model to briefly acknowledge the user's tense/anxious mood — matching the plan's own worked example, without inventing a new detector.
5. **Gossip reachability** — when the shared `CLASSIFY_ADVISOR_TURN` classifier (already built in Phase 1) returns `intent: "gossip"`, both `handle_turn()` and `handle_conversation_turn()` now switch `companion_mode` to `gossip` for that turn even without the chip being tapped — this is the "orchestrator's own judgment" half of §3.7; the automatic Gossip Worthiness Detector that proactively *initiates* a gossip conversation is still Phase 4.5.
6. **`proposedAction`/`is_high_risk_draft` are computed but not yet acted on.** When intent is `draft_reply`/`send_message` and a contact is in scope, `handle_conversation_turn()` already returns a `proposedAction: {actionType: 'send_whatsapp_message', payload, riskLevel}` object, and the analysis prompt already self-assesses `is_high_risk_draft`. Nothing on the Node or frontend side reads this yet — Phase 3 is what persists it into `advisor_action_requests` and builds the actual approval flow. Shipping the computation now (inert until Phase 3) avoids re-deriving the same drafted-text risk signal a second time next phase.
7. **Frontend evidence cards live in the Inbox's `intel-panel.tsx`** — the real per-conversation Advisor surface (not the global `/advisor` page, which has no single active conversation by default). `advisor_messages.metadata` now also stores `analysis` alongside `assistantState`/`memorySuggestion` so reloading chat history still shows the evidence card on past messages, not just the live turn.

### Phase 3 — Action Protocol And Approval

**✅ Shipped** (migration `0068`). See `CLAUDE.md`'s "Advisor Companion" section for the full summary.
...
1. **`advisor_action_requests` ships with the full §4.9 action-type list already widened in** — `send_devotional`/`send_motivational`/`send_interest_update` are in the CHECK constraint from day one (this is a fresh table, not an ALTER against existing rows, so there's no reason to widen it later); only `send_whatsapp_message` execution is actually implemented — the other two need Phase 4.5's crons to exist before there's anything to send.
2. **Execution reuses the existing WhatsApp send queue**, per the plan's own implementation note (§10). `services/api/src/lib/whatsapp-send.ts`'s `sendWhatsAppMessage()` is extracted out of `POST /api/conversations/:id/messages` (which now just calls it) so `POST /api/advisor/actions/:id/execute` calls the exact same path instead of a parallel sender.
3. **The Boundary Keeper (§3.11/§6.13) is now a real 3-factor check**, not just the content self-assessment Phase 2 shipped: `AdvisorCompanionService._assess_boundary_risk()` combines (a) the model's own `is_high_risk_draft` judgment (Phase 2), (b) the user's current arousal being elevated (reusing Phase 2's threshold), and (c) whether this specific contact has had a recent (14-day) negative average valence in `emotional_signals`. Two or more factors → `risk_level = 'high'` (the "want to sleep on it?" prompt); one → `medium`; zero → `low`. Risk level never blocks sending — approval is always required regardless, exactly per §8.1's baseline policy; risk level only changes what the approval card *shows*.
4. **Approve and execute are two separate endpoints** (`POST /api/advisor/actions/:id/approve`, `POST .../execute`), matching §5.3's route list, but the frontend's single "Send Now" button calls them back-to-back — there's no product reason to force a user through two clicks for one action today. `POST .../cancel` is the third terminal transition.
5. **The approval card lives in `intel-panel.tsx`** (same surface as Phase 2's evidence cards) with an editable draft, a Send Now / Edit / Cancel row, and the amber "want to sleep on it?" note only when `riskLevel === 'high'`. `advisor_messages.metadata.actionRequestId` lets a reloaded chat history reconstruct the card by re-fetching `GET /api/advisor/actions?sessionId=`.
6. **Only `send_whatsapp_message` executes today.** `POST .../execute` returns a 400 for every other action type — `create_reminder`/`generate_document` already have working equivalents through the existing `[ACTION: ...]` chat-tag system, so building a second execution path for them through this table isn't needed yet; `fetch_replies`/`watch_conversation`/`summarize_new_replies` are Phase 4 (Watch Replies And Narration) scope.

### Phase 4 — Watch Replies And Narration ✅ Shipped
...
1. **No new migration.** §5.4's `POST /api/advisor/watch` / `DELETE /api/advisor/watch/:id` are backed by the same `advisor_action_requests` table Phase 3 shipped, reusing its already-unused `watch_conversation` action type instead of a dedicated table — a watch row is inserted `status = 'approved'` directly (there's nothing risky to approve about a passive watch, unlike a send) with `expires_at` set from an `expiresInMinutes` param (default 60). A `GET /api/advisor/watch?sessionId=` endpoint was added beyond §5.4's literal list so the frontend toggle and a reloaded chat history can both reconstruct current watch state.
2. **Two ways to start a watch, one storage path.** The frontend's Watch/Watching toggle in `intel-panel.tsx`'s chat tab calls the REST endpoints directly; saying "watch this chat and tell me when they reply" in the chat itself is handled as a `watch_replies`-intent short-circuit in `AdvisorCompanionService.handle_conversation_turn` (same pattern as Phase 1's `activate_personal_mode` short-circuit) that calls the same `_create_watch()` helper — both paths write the identical `advisor_action_requests` row, so either one is visible to the other.
3. **Narration + suggested replies are one combined LLM call**, not two — `NARRATE_REPLY` (`ai/prompts.py`) returns `{narration, suggested_replies}` in a single structured completion, deliberately not reusing `reply_gen.py`'s heavier `ReplyGenerator.generate()` pipeline (which has its own DB-write and auto-response side effects unrelated to a watched-conversation narration), per the research done before implementation.
4. **The "warmer than last week" read is grounded in two already-computed signals**, not a new detection pass: `AdvisorCompanionService._get_reply_trend_context()` compares trailing-7-day vs prior-7-day average `emotional_signals.valence` for that contact, and folds in `relationships.health_trend` if it's `improving`/`declining`. Genuinely emergent trends get named; a quiet week produces no trend line rather than a forced one.
5. **The hook lives in `message_worker.py`**, right after the existing order-intent action-bundle detection (inside the same `if sender_type == 'contact' and body:` block, historical-message-safe by the same `not is_historical` guard everything else in that block uses) — `find_active_watch()` checks for an approved, unexpired watch on the conversation; if found, `advisor.reply_received:{userId}` fires immediately (a fast, narration-less ping) and `advisor.narration_ready:{userId}` follows once the narration call completes, both added to `redis-subscriber.ts`'s explicit psubscribe pattern list per the existing `suggestion:ready`/`bundle:ready` convention. The narration is also persisted as a normal `advisor_messages` row (`metadata.type = 'narration'`) so it survives a chat-history reload.
6. **The narration card renders inline in `intel-panel.tsx`'s chat tab** (violet-tinted, distinct from the indigo evidence card and emerald approval card) with suggested replies as tappable chips that populate the reply box via the existing `onSetDraft`/`draftFocus` props — there's no separate notification surface yet (§7.7's "Zuri Noticed Something" card is Phase 4.5 scope); a user who isn't looking at the Inbox won't see this until they open it.
7. **The Watch toggle is disabled until a chat session exists** — `chatSessionId` is only created once the first Advisor message is sent in that conversation (existing Phase 1 behavior), so watching before asking anything isn't possible from the UI toggle; the chat-intent path has the same constraint since it also needs a `session_id`.

**Phase 4.5 — Proactive Companion Crons ✅ Shipped**
...
1. **Migration `0069`** adds `proactive_interest_chats`/`gossip_worthy_events` exactly per §4.7/§4.8, plus `advisor_messages.initiated` (the §6.1 orchestrator-note column that lets a proactive message be told apart from a reply).
2. **All four crons are gated on `personal_mode_enabled = true` (not organic discovery)** — this session never built a literal discovery-tracking mechanism (no table of "has this user used gossip mode before"), and §1.2 itself explains `personal_mode_enabled` exists precisely so these crons are testable end-to-end without waiting on that; the Spiritual Companion is the one exception, gated purely on `spiritual_preferences.tradition` being set per its own hard consent rule (§3.9), independent of personal mode. `companion_features_paused` (§7.8) still overrides all four regardless.
3. **Gossip detection is 5 real SQL aggregations**, `services/intelligence/app/services/gossip_detector.py`: `tone_shift` (7-day-vs-prior-7-day `emotional_signals.valence` delta ≥0.25), `ghosting` (`relationships.last_interaction_at` more than 2× `relationship_clocks.avg_days_between_messages` stale), `sudden_interest` (this week's contact-sent message count ≥1.7× the cadence-implied expectation), `life_event` (a `contact_life_events` row from the last 3 days), `reciprocity_drop` (`relationships.health_score` down ≥10 points vs. a `relationship_health_logs` row from 7+ days ago) — same "read signals that already exist" discipline as `pricing_benchmarks.py`. A contact+signal_type combination won't re-fire within 3 days. Delivery timing (§6.9's "is now a good moment" check) is NOT in Python — it lives in Node's `GET /api/advisor/companion-feed`, which already owns this read path and can check `advisor_user_profiles.current_emotional_state` directly (valence ≥ -0.2 and arousal ≤ 0.6) without a second service hop; each call flips at most one highest-confidence pending row to `delivered`.
4. **The Proactive Interest Cron's search is one tier, not three.** §6.10 calls for a hybrid chain (model-native search tool → `web_search.py` → forced Gemini). Only `web_search.py` (Tavily/SERP) is wired up — `ai/client.py` has no LiteLLM tool-calling/grounding support for any provider today, and building an unverified new grounding integration with no way to exercise real tool-call responses in this environment would be exactly the "shipped code that can't be verified" mistake this project avoids elsewhere. If no `TAVILY_API_KEY`/`SERP_API_KEY` is configured, a topic is silently skipped for that cycle — the same graceful no-op `interest_matcher.py` already uses when it has no headlines. Topics come from `advisor_user_profiles.interests` plus the close circle's (`importance_tier IN (1,2)`) aggregated `contact_insights`, capped at 5 per run, deduped per-topic within a 20-hour window.
5. **Interest/devotional/motivational deliveries reuse one shared helper**, `services/intelligence/app/services/companion_delivery.py`'s `deliver_initiated_message()` — resolves (or creates) the one global Advisor session, inserts an `advisor_messages` row (`initiated = true`), and bumps session counters, so `interest_companion.py`/`spiritual_companion.py`/`motivational_detector.py` don't each duplicate that logic. Gossip is deliberately NOT delivered this way — it stays in `gossip_worthy_events` until the companion-feed's delivery-timing check surfaces it, since a gossip observation needs the "Tell me more" framing rather than showing up as a pre-written chat message.
6. **The devotional and motivational nudge are wording-only LLM calls.** `GENERATE_DEVOTIONAL`/`GENERATE_MOTIVATIONAL_NUDGE` (`ai/prompts.py`) never detect anything — the devotional cron only checks `spiritual_preferences.tradition` is set and a 20-hour dedup window; the motivational detector's signals (contacts unreplied 48h+ via a `LATERAL` join on each conversation's last message, unfulfilled `message_analyses.promises_detected` older than 48h, `deals` stuck in the same `stage` 7+ days via the already-existing `entered_stage_at` column) are plain SQL, requiring ≥2 to fire — same discipline as `pricing_benchmarks.py`/`document_followups.py`.
7. **Context-sensitive verse offering and "pray with me" are inline, not a cron**, exactly as §6.11 specifies. `AdvisorCompanionService._spiritual_verse_line()` folds one extra instruction into the system prompt (both `handle_turn` and `handle_conversation_turn`) when a tradition is set AND the same low-valence/high-arousal state Phase 2's `emotional_context_line` already checks for is present — the model *may* offer a verse, never forced. The `spiritual` intent (already in `CLASSIFY_ADVISOR_TURN`'s enum since Phase 1) now auto-switches `companion_mode` to `spiritual_companion` for that turn, mirroring exactly how the `gossip` intent already auto-switches — but only once a tradition is set, honoring the consent gate.
8. **The Companion Preferences API (§5.5) isn't duplicated.** `GET/PATCH /api/advisor/profile`, shipped in Phase 1, already returns/updates `interests`/`spiritualPreferences`/`motivationalStyle`/`gossipStyle`/`companionFeaturesPaused` — building a second endpoint for the exact same fields would be pure duplication. Only the companion-feed half of §5.5 (`GET /api/advisor/companion-feed`, `POST .../:id/dismiss`) is new.
9. **The "Zuri Noticed Something" card only surfaces gossip items.** Interest/devotional/motivational nudges already arrive as ordinary `advisor_messages` rows the user sees the moment they open their Advisor session — they don't need a second delivery surface. The card (violet-tinted, on `/advisor`, above the message list in both empty and active states) is gossip-only: "Tell me more" switches `companionMode` to `gossip` and sends a pre-filled question naming the contact and the grounded observation; "Not now" calls the dismiss endpoint. No push notification exists for a user who isn't looking at Advisor — per §7.7's own instruction ("never a push notification with invented urgency"), that's out of scope here.
10. **No per-user timezone scheduling.** §3.8's "user's own timezone" and §3.9's "user-configured time" are both simplified to fixed UTC hours (interest cron every 6h; gossip detection 12:00 UTC; devotional 13:00 UTC; motivational 14:00 UTC, staggered after the existing Neural Layer/Business OS daily jobs) — no scheduler in `daily_worker.py` does per-user timezone offsetting today, so this stays consistent with every other cron in that file rather than being a bespoke exception.

### Phase 5 — Learning Loop And Personalization ✅ Shipped
...
1. **No new migration** — the learner reads/writes tables that already exist (`suggested_replies`, `advisor_action_requests`, `advisor_memories`, `advisor_user_profiles`, `proactive_interest_chats`).
2. **Only the genuinely-missing trigger point was built.** §6.5 lists three: "after each advisor turn" (already existed — Phase 1's `CLASSIFY_ADVISOR_TURN` proposes a `memory_suggestion` on every turn) and "after explicit feedback" (already existed — the remember/correct/forget endpoints are already fully-trusted immediate writes) needed nothing new. Only "nightly consolidation" was missing; `services/intelligence/app/services/advisor_memory_learner.py` adds it, wired into `daily_worker.py`/`main.py` at 15:00 UTC (after the Phase 4.5 motivational check at 14:00) — same pattern as `neural/emotion.py`'s reconsolidation job, applied to `advisor_memories` instead of `emotional_signals`.
3. **Tone-preference learning reuses `suggested_replies`**, which already records exactly what §6.5 asks for ("accepted/rejected suggestions") via its `status` column (`approved`/`edited_and_sent`/`dismissed`) — no new instrumentation needed. A tone only gets recommended once it has ≥5 samples in the trailing 30 days and scores ≥0.5 (approved=1.0, edited=0.5, dismissed=0.0), upserting both `advisor_user_profiles.tone_preferences.preferredReplyTone` and a matching `advisor_memories` row — never overfitting from one message, per §6.5's own rule.
4. **"Approved drafts" learning reads `advisor_action_requests`** (Phase 3/4's table): a ≥40% cancel rate over ≥5 recent `send_whatsapp_message` proposals writes a `disliked_advice` memory flagging that drafts aren't landing — a concrete, low-risk signal rather than trying to infer *why* from the cancelled text itself.
5. **Weak-memory deactivation** mirrors `emotion.py`'s reconsolidation exactly: a memory with `confidence < 0.3` AND unreinforced for 30+ days AND `evidence_count <= 1` gets `is_active = false` — the `evidence_count <= 1` guard means anything a user has ever corrected or that has accumulated repeat evidence is protected from ever being silently dropped.
6. **Engagement-based frequency tuning for the Phase 4.5 crons is now real**, closing a gap Phase 4.5 itself left open (its own success criterion promised "dismissing/ignoring it visibly reduces frequency," which wasn't actually wired until now). `companion_delivery.py`'s new `engagement_rate()` reads `proactive_interest_chats.user_engaged`; `interest_companion.py` skips its entire run for a user once combined engagement across its four content types drops below 15% (≥5 samples), `motivational_detector.py` does the same for `motivational` specifically, and `gossip_detector.py` skips a user's detection pass entirely once their dismiss rate across the last 14 days exceeds 70% (≥5 resolved events). `user_engaged` itself is now actually set — `POST /api/advisor/sessions/:id/messages` (Node) checks whether the session's last message was `initiated = true` before persisting the new user turn, and if so marks that session's most recent `proactive_interest_chats` row engaged.
7. **Deliberately not built**: the associative emotional graph for pattern surfacing ("you've had three conversations like this...") and per-content-style learning (which specific devotional translation, which specific gossip tone phrasing resonates) — both would need materially more infrastructure (graph traversal over `emotional_signals`, finer-grained content tagging than `content_type` currently provides) than a nightly aggregation pass can responsibly deliver. What shipped is frequency-based learning (does the user want *more or less* of this) and tone-based learning (which *reply tone* works) — genuinely useful, verifiably grounded in existing data, and honestly scoped short of the plan's full ambition here.

### Phase 6 — Safe Scoped Automation ✅ Shipped
...
1. **Migration `0070`** adds `advisor_automation_grants` (one active grant per conversation at a time, `scope_description` in the user's own words, `expires_at`) and `advisor_automation_audit_log` (`auto_sent`/`skipped_out_of_scope`/`skipped_high_risk` rows per candidate message).
2. **"Integrate with existing auto-response trust tiers" is literal, not aspirational** — a grant never bypasses `auto_response.py`'s `check_eligibility()` (business hours, per-contact/rule exclusions, escalation keywords, group/broadcast skipping, the master on/off switch all still apply exactly as they do for every other auto-send path). `services/intelligence/app/services/reply_gen.py` now calls `check_eligibility()` directly (instead of `evaluate()`, which bakes in the `approval_mode == 'auto'` requirement) and only reaches for an active scoped grant when eligibility passes but the account's global `approval_mode` would otherwise block the send — a grant only ever overrides that one gate.
3. **Per-message re-classification, not a static rule.** `services/intelligence/app/services/scoped_automation.py`'s `check_reply_in_scope()` re-checks every candidate reply against the grant's stated scope via `CLASSIFY_SCOPED_AUTOMATION` (`ai/prompts.py`) — the grant's own scope description is never trusted alone to greenlight a specific exchange, and `is_high_risk` always wins over `in_scope` regardless of what the grant nominally covers (money, commitments, complaints, anything emotionally charged always gets skipped and logged, never auto-sent).
4. **Creation is symmetrical to Phase 4's watch, revocation is REST-only.** A grant can be created via the panel (`POST /api/advisor/automation-grants`) or by saying "handle this conversation for 10 minutes, auto-send only logistical confirmations" — a new `scoped_automation` intent in `CLASSIFY_ADVISOR_TURN` short-circuits in `handle_conversation_turn` the same way `watch_replies` does, using the literal message as `scope_description` and a fixed 30-minute default (the panel supports 15/30/60). Revocation only exists as `POST .../:id/revoke` — no chat-intent counterpart, matching the same asymmetry Phase 4's watch already established.
5. **The audit log is a real, queryable record**, not just a log line: every candidate exchange while a grant is active writes one `advisor_automation_audit_log` row (what was sent, or why it was skipped) via `GET /api/advisor/automation-grants/:id/audit-log`, surfaced inline in `intel-panel.tsx`'s Automate panel (violet-tinted, matching Phase 4's watch/narration and Phase 3's approval-card conventions) alongside the scope description, expiry time, and a Revoke button.
6. **No new send mechanism.** `enqueue_send()` (already built for the plain auto-response path) is reused verbatim — a scoped-automation auto-send is indistinguishable from a normal `approval_mode = 'auto'` send once it clears the eligibility + scope + risk gates, which is the intended behavior: this is "temporarily borrow the auto tier for one narrow purpose," not a parallel execution path.
