...
**Status (2026-07-14):** Planning document. Nothing in this file has shipped yet — it is the forward-looking companion to `docs/STUDIO_ERP_PLAN.md` (which specifies what **already exists**: `suppliers`, rich `products` columns, `stock_movements`, `GET /api/studio/insights`, and the Studio AI Business Advisor with `[ACTION: ...]` tags — all live in production as of migration `0055`). This document starts from that shipped foundation and defines the next several phases that turn Studio from "a good catalog + inventory + advisor" into the thing the product should actually become: the AI layer that keeps a business's operational reality in sync with what happens in its conversations, without anyone doing manual data entry. Every proposal below is labeled **extends** (builds on a shipped table/engine), **reconciles** (fixes a naming/design inconsistency along the way), or **net-new** (nothing like it exists yet) relative to the audit in §2. No implementation has started; this is the plan to review and phase.
...
Explicitly the lowest-priority item in this plan — the user framed it as "eventually." Minimal viable design when it's time:
...
|---|---|---|
| **A** | Product families + attribute definitions (§5) — schema + "Manage Product Type" UI + variant generation | Nothing new; extends `products` |
| **B** | Purchase order workflow (§8.3) + supplier-products pricing (§8.2) | Phase A not required, can ship in parallel |
| **C** | Multi-location inventory (§7.1) + expanded movement vocabulary (§7.2) | Phase A not required |
| **D** | Sales intelligence surfacing — co-purchase, cross-sell/upsell UI (§9) | None (data already exists) |
| **E** | `action_bundles` + transactional-intent detector + Inbox bundle-approval card (§15) — **the core differentiator, highest priority despite being listed after the infra phases** | Benefits from Phase A/B/C existing but can ship a v1 against today's flat schema and be widened later |
| **F** | Projects module (§11) + AI Project Manager brief | None |
| **G** | Inventory forecasting (§7.3) + operational financial overview (§13) | Needs 2-3 months of real `stock_movements` history to be useful — sequence last on purpose |
| **H** | Manufacturing/BOM (§12) | Build only when a real user asks |
...
4. **Expenses storage.** §13 leaves open whether to build a dedicated `expenses` table or reuse the `expense_claim` document type — decide when Phase G is actually scheduled, not now.
5. **How aggressively `action_bundles` auto-executes vs. always requires approval.** The vision text says "pending approval or automatically, based on user preferences" — this needs a trust-level concept for the bundle detector analogous to the Autonomous Agent Engine's Observe→Autonomous dial, but scoped to inventory/document actions specifically. Needs explicit design before Phase E ships, not left implicit.
