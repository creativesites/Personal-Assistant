...
**Phase 6 — ATS Analysis + CV Health** ✅ **Shipped** (§7) — ATS Analysis (`POST /api/career/cvs/:id/ats-score`) reuses `score_resume_text()`'s existing `SCORE_RESUME` prompt verbatim through the already-built `/internal/career/resume/score` intelligence endpoint — Node's new `buildCvPlainText()` (`services/api/src/lib/cv-health.ts`) is the only new piece, serializing the CV's live relational data into the plain text that prompt expects. CV Health (`GET /api/career/cvs/:id/health`) is a fully deterministic, non-AI scorer (`computeCvHealth()`) checking: missing LinkedIn URL, no quantified achievements (regex for digit/%/currency across achievement text), summary too short/too long/missing, an unexplained 90+ day gap between employment entries, an estimated-too-long CV (line-count heuristic), zero certifications, and weak-verb phrasing ("responsible for", "worked on") — each deducting fixed points from a 100 baseline, each carrying its own one-tap fix-action description. Frontend: a new `CvHealthPanel` renders below the live preview in the wizard — CV Health loads automatically (cheap), ATS Analysis is a button-triggered on-demand AI call showing all five sub-scores plus concrete rewrite suggestions.

**Phase 7 — Web Editor + Version History** ✅ **Shipped** (§9, §10): a distinct post-wizard page (`career/cv-studio/[id]/editor/page.tsx`, reached via an "Editor" button in the wizard's header) rather than a 15th wizard step — the plan frames the Web Editor as a separate editing mode over an already-populated CV, not part of the linear content-entry flow. `web-editor-panel.tsx`'s `WebEditorPanel` renders/reorders/hides/renames `career_cv_sections` against the exact `PUT /api/career/cvs/:id/sections` whole-list-replace endpoint Phase 1 already shipped — no backend changes needed. Reorder is up/down-arrow buttons, not drag-and-drop: this codebase has no drag/sortable library anywhere yet, and pulling one in solely for this one panel isn't worth the new dependency — arrows are a fully working, keyboard-accessible mechanism that needs none. `version-history-panel.tsx`'s `VersionHistoryPanel` lists `GET /api/career/cvs/:id/versions` (Phase 1), restores via `POST .../versions/:versionNumber/restore` (a non-destructive copy-forward, never overwrites history), and duplicates the whole CV via `POST .../duplicate`, navigating straight into the new copy's wizard. "Compare" is a deliberate simplification of §10's fuller vision: a plain side-by-side JSON snapshot display for two checkbox-selected versions, no line-level diff highlighting — an honest scope reduction, not a real diff algorithm, matching this codebase's "ship a real slice, document the rest" discipline. The editor page still renders the live `CvPreview` alongside both panels, per §9's "still live-preview-backed" requirement.

**Phase 8 — Tailored Variants + Job Matching UI** ✅ **Shipped** (§8, §11): the CV-to-opportunity loop, surfaced on the opportunity card. "Create Variant" needed no backend work — `POST /api/career/cvs` has taken `sourceCvId`/`careerOpportunityId` since Phase 1 — only a frontend entry point: `CvTailoringPanel` (new file, `career/_components/cv-tailoring-panel.tsx`, sibling to `opportunity-insights.tsx` rather than another panel piled into `opportunity-card.tsx`) creates a variant from the user's master CV with `careerOpportunityId` set and navigates straight into its wizard. Job Matching (§11) is a new intelligence service, `cv_matching.py`: `compute_cv_opportunity_match()` embeds the CV's live text (assembled by Node's existing `buildCvRenderData()`/`buildCvPlainText()` — the same functions CV Health already uses, so no CV-assembly logic is duplicated in Python) against the opportunity's description using the exact cosine-similarity mechanism `resume_studio.py`'s `match_resume_to_opportunities()` already established, plus one structured extraction call (`EXTRACT_JOB_REQUIREMENTS`) pulling only the skills/technologies the posting text itself states — diffed against the CV's own skill groups to produce a missing-skills list. `generate_tailoring_suggestions()` is the "Generate Improvement Suggestions" action the plan calls for by name (never "Generate Fake Experience"): one `SUGGEST_CV_TAILORING` call, prefixed with `CV_STUDIO_NEVER_INVENT_POLICY`, proposing which of the CV's *existing* bullets/skills to emphasise or reorder for this specific opportunity — grounded only in text already present in the CV. `services/api/src/routes/career-cv-matching.ts` (a new sibling file to `career-cvs.ts`, which was already ~400 lines) proxies both: `GET /api/career/opportunities/:id/match/:cvId` and `GET .../tailoring-suggestions?cvId=`.

**Phase 9 — Cover Letter Studio + Supporting Documents** ✅ **Shipped** (§12, §13): migration `0082` widens `documents.document_type`'s CHECK with `application_letter`/`expression_of_interest`/`personal_statement`/`motivation_letter`/`reference_sheet`/`portfolio_pdf` (the same widen precedent every prior document-type addition in this codebase used). Cover Letter Studio is deliberately **not** a second invent-from-instruction generator — the pre-existing `/api/career/cover-letter` (Career Growth Engine Phase 3) is left exactly as-is, since Resume Studio's own "Generate Cover Letter" button still calls it and deleting it now would regress a live feature with nothing forcing the change. Instead, a new **compose-and-save** path is the plan's actual ask: `resume_studio.py`'s `create_resume_document()` gained a generic `_DOCUMENT_TYPE_LABELS` map (was hardcoded to two types) and a new shared `/internal/career/documents/save` route accepts any of the six new document types plus `cover_letter` — one route rather than six near-identical ones, since only `structured_data` differs and Node already assembles it. `services/api/src/routes/career-documents.ts` gained `POST /api/career/letters/compose` (recipientName/companyName/body/signOff — the body is real, user-composed text, optionally AI-polished via the pre-existing generic `/api/career/cv-assistant/rewrite` endpoint, never generated from a free-text instruction), `POST /api/career/reference-sheet` (a plain, non-AI rendered view of `career_references`), and `POST /api/career/portfolio-pdf` (a plain, non-AI rendered view of `projects.is_portfolio_visible = true` rows) — the latter two need zero user input beyond an optional title. Two new React-PDF templates, `ReferenceSheet.tsx` and `PortfolioPdf.tsx`; the four letter-shaped Phase 9 types reuse `renderCoverLetterPdf()` verbatim (identical `{recipientName, companyName, body, signOff}` shape) rather than a duplicate template. `document-render.ts`'s career-document branch (previously `resume`/`cover_letter` only) now covers all eight types. Frontend: a new `CoverLetterStudio` component (`career/_components/cover-letter-studio.tsx`, its own file per the File Architecture convention — `resume-studio.tsx` was already ~300 lines) renders alongside Resume Studio on `/career`: "Compose a Letter" opens a modal listing the user's own real employment achievements as one-tap-to-insert chips (plain data, no AI) above a body textarea with the existing `AiRewriteToolbar` for polish-only rewriting; "Reference Sheet"/"Portfolio PDF" are one-click generate buttons. The plan's own (b) "Zuri suggests surfacing achievements for a specific opportunity" is deliberately *not* wired into this modal as a separate AI call — Phase 8's `generate_tailoring_suggestions()` already does exactly this, but scoped to a `career_cvs` row via `cvId`, which a from-scratch letter compose doesn't have; forcing that fit would have meant either a fake CV requirement or a second, parallel suggestion mechanism, so this phase ships the achievement-picker (real, deterministic, always available) and leaves "AI-ranked which achievement to lead with" as a documented follow-on once it's clear a compose-time CV context is worth requiring.

With Phase 9 shipped, all nine phases of `docs/CV_STUDIO_PLAN.md` §18 are complete. Each phase shipped independently useful value, per this codebase's own established discipline.
