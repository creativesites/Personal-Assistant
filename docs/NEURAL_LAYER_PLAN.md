...
**✅ Shipped** (migration `0065`). See `CLAUDE.md`'s "Zuri Neural Layer" section for the summary of what's live.
...
**Scope note:** the co-purchases consumer lives in `services/api` (Node), while the plan's own `neural/knowledge_graph.py` naming assumes Python (`services/intelligence`) — since the two services keep independent DB pools and neither calls the other's code (same split as everywhere else in this codebase), the traversal contract was implemented twice with the same shape: `services/api/src/lib/knowledge-graph.ts` (the actual dependency of the co-purchases endpoint) and `services/intelligence/app/neural/knowledge_graph.py` (matching the plan's naming, and now the one place `retrieval_service.py::get_co_purchases` — used by `reply_gen.py`'s catalog context — delegates to, removing what used to be a second copy of the same SQL). Both read/write the same `knowledge_graph_edges` table. Only the `contact_products`-derived co-purchase traversal is implemented so far; the table's `inferred_neighbors`/`inferredNeighbors` read path has no writer yet — populating it is follow-up work for whichever engine first needs an edge no FK already expresses.

### Phase 5 — Prediction Engine Consolidation

**✅ Shipped** (no new migration — read-only adapters over already-materialized tables). See `CLAUDE.md`'s "Zuri Neural Layer" section for the summary of what's live.
...
**Scope note:** `PredictionEngine.predict()` dispatches by `prediction_type`: `stockout` reads `inventory_forecasts` (confidence hardcoded at 0.7 since that job doesn't produce its own confidence value), `renewal_due`/`churn_risk` both read the matching open `opportunities` row by `opportunity_type` (confidence/evidence taken directly from that row), and `purchase_likelihood` is the new heuristic — a deterministic score (not an LLM call, same convention as `pricing_benchmarks.py`) over recent `contact_products` quote/interest signals and `relationships.health_trend`. Exposed via `POST /internal/predictions/:predictionType` in `services/intelligence` and proxied through `GET /api/predictions/:predictionType/:subjectId` in `services/api`; a "Purchase Likelihood" card renders on `/contacts/[id]` (business/hybrid mode only) showing the percentage and evidence bullets. This confirms the contract's value: `purchase_likelihood` needed ~60 lines of new logic versus the ~40-line adapters wrapping three already-built predictors — the win is a consistent shape any future engine can read, not less code for this one prediction.
