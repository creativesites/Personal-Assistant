...
**6.5 — Shipped, and the migration pattern to follow going forward.** `services/intelligence/app/memory/retrieval_service.py` gained `get_open_opportunities`/`get_project_status`/`get_invoice_aging`/`get_deal_pipeline_summary`, symmetric to its existing contact-entity fetchers. `services/intelligence/app/services/business_context_service.py`'s `BusinessContextService` has two entrypoints: `answer(surface, user_id, question, scope, chat_history, extra_context_blocks, system_suffix)` — the full one-call path (context assembly + one `complete_text` call) for a surface whose response is plain text — and `get_context_block(user_id, contact_id)`, the shared business-entity text block exposed separately for a surface whose response shape doesn't fit a single generic completion. `routes/conversation.py::studio_ask` is the first proof, fully migrated onto `answer()` — its own supplier/low-stock/recent-contacts blocks and `ZURI_ACTION_INSTRUCTIONS` pass through as `extra_context_blocks`/`system_suffix` rather than duplicating the shared catalog/facts/business-entity assembly. `advisor_companion.py::handle_conversation_turn` is the second proof: for `chat_analysis`/`relationship_advice`/`emotional_support` intents only (skipped otherwise, so a routine chit-chat turn doesn't pay for context it won't use), it calls `get_context_block()` and folds the result into its own `contact_context` string — a deliberately lighter integration than `answer()`'s single-call path, since this service already assembles a structured, multi-signal prompt (`ANALYZE_CHAT_TURN`) of its own that a second generic completion call would duplicate rather than simplify. **The pattern for every new engine going forward**: if it needs "what does Zuri already know about this business" context, add a fetcher to `retrieval_service.py` if one doesn't exist, then either call `BusinessContextService.answer()` (plain-text surfaces) or `get_context_block()` (surfaces with their own structured prompt) — never re-derive an ad-hoc opportunities/projects/invoices/deals query at the call site. Enforced by convention/code-review per §6.4, same as every other cross-cutting discipline in this codebase.
...
**Shipped (migration `0089`).** §7.1: `_period_bounds()` gained a `quarterly` branch (90 days) alongside the now-wired `monthly` one; two new daily schedulers (`reflection.generate_monthly` on the 1st of each month, `reflection.generate_quarterly` on the 1st of Jan/Apr/Jul/Oct, both 11:00 UTC) call `generate_for_all_users('monthly'/'quarterly')`. `_build_highlights()` gained an invoices-sent/paid-this-period highlight and a sales-trend-vs-prior-period highlight (both plain aggregations against `documents`/`revenue_events`, same pattern as every existing category), plus the one correlational insight (`_quote_latency_conversion_insight()`): quotations bucketed by sent-minus-created latency (<30min vs. not), correlated against whether that contact's deal ever closed won — surfaced only when the fast bucket's conversion rate is actually higher and both buckets have ≥3 samples, never invented.

§7.2 shipped 5 of the suggested ~6-8 detectors, an honest bounded subset, not a silent cap: **payment-posted** and **milestone-counter-crossing** (Nth invoice paid / Nth deal closed, exact-match against `{5,10,25,50,100,250,500,1000}`) live in Node (`services/api/src/lib/business-feed.ts`'s `recordBusinessEvent()`/`checkMilestoneCrossing()`) since `documents.ts`/`deals.ts` already own those writes; **project-percent-crossing at 100%** (the 75% threshold already had a home in `project_progress.py`), **repeat-product-mention** (a product `contact_products` distinct-contact count crossing the same round thresholds), and **contact-gone-quiet** (a contact whose messaging cadence has gone unusually silent — detect-and-surface only, no nudge, same posture as Reality Engine's contradictions) live in the new `services/intelligence/app/services/business_feed_detectors.py`, wired as a twelfth daily scheduler at 22:00 UTC. Explicitly not shipped this pass: period-over-period sales-trend as its own *feed* event (it's a Reflection highlight, §7.1, not a Business Feed detector — the plan's own item list conflated the two). `GET /api/business-feed` (cursor-paginated, `services/api/src/routes/business-feed.ts`) and the first-class `/feed` page ship as designed; the label map previously inlined in `studio/page.tsx` moved to `apps/web/src/lib/business-event-labels.ts` so both surfaces read one source of truth.
