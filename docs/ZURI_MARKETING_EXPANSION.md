...
**Pre-launch testing overrides.** Several access-control relaxations, all explicitly temporary:
...
- Migration `0035` sets `users.marketing_access`'s default to `'enabled'` and backfills every existing row.
- **Root cause found**: migration `0035` (and every migration before it) never actually reached the production database. `services/api/Dockerfile` never `COPY`s the `db/` directory into the image, and `.github/workflows/deploy.yml` never ran `db:migrate` at all — there was simply no path by which a migration file landed on production. Fixed the workflow (see below) and, since a broken migration pipeline shouldn't be a single point of failure for whether this feature is visible, also hardcoded the override in code: `services/api/src/lib/marketing-access.ts` exports `MARKETING_ACCESS_OPEN_FOR_TESTING = true`, which `requireMarketingAccess()` and every `marketingAccess` field `services/api/src/routes/auth.ts` returns (`clerk-sync`, `/me`, `PATCH /users/me`) now honor unconditionally, regardless of the DB value. Flipping this one constant back to `false` (once real launch approaches) restores everything to reading from the DB — the column, `FeatureGate`, and all of §12's gating logic were never touched.
- `.github/workflows/deploy.yml` now runs migrations on every deploy, in a throwaway `node:22-alpine` container against the checked-out repo (mounted, not baked into any image) with `--env-file .env`, right after pulling code and before restarting the app containers. This closes the actual gap — future migrations will reach production automatically on push to `main`.
